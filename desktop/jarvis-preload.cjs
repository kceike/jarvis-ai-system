...
contextBridge.exposeInMainWorld("jarvisDesktop", Object.freeze({
  ...
  searchLocalFiles: (query) => ipcRenderer.invoke("jarvis:search-local-files", String(query || "").slice(0, 160)),
  askLocalAI: (payload) => ipcRenderer.invoke("jarvis:ask-local-ai", payload && typeof payload === "object" ? payload : {}),
  analyzePsd: (payload) => ipcRenderer.invoke("jarvis:analyze-psd", payload && typeof payload === "object" ? payload : {}),
  analyzeOffice: (payload) => ipcRenderer.invoke("jarvis:analyze-office", payload && typeof payload === "object" ? payload : {}),

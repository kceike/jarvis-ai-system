...
JARVIS 1.13.7 includes a secure Electron desktop wrapper and standard electron-builder configurations for two Windows formats:

- `JARVIS-AI-Setup-1.13.7-x64.exe` — assisted NSIS installer for normal personal installation.
- `JARVIS-AI-1.13.7-x64.msi` — WiX/MSI installer for IT deployment, SCCM, Intune, Group Policy, and standard `/quiet` or `/passive` installation.
...
The v1.13.7 upgrade routes Gemini text and code through the current Interactions API, makes the connection test verify a real response, recovers interrupted requests visibly, and retains Unified Brain fallback, text-plus-voice replies, every free-first capability, and every safeguard.

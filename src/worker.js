    ...
    (function(){
      ...
      var HELP_SECTIONS=[
        ...
        {title:"ADVANCED INTELLIGENCE AND MULTIMODAL COMMANDS",items:[
          {name:"Auto Director",description:"Select Auto Director in Settings to route ordinary chat to a balanced model, difficult analysis to a reasoning model, and programming work to the coding model. Fixed Cloudflare models remain selectable."},
          ...
          {name:"Cloud neural voice",description:"Enable Cloud neural voice in Settings for generated English speech. If the neural voice is unavailable, JARVIS safely falls back to the installed Windows or browser voice."},
        ...
        try{
          ...
          if(c.mode==="image"){
            ...
            data=await postJson("/api/chat",{
              mode:c.mode,modelKey:state.settings.cloudModel,reasoningPower:state.settings.reasoningPower,contextWindow:state.settings.contextWindow,temperature:state.settings.creativity,concise:state.settings.concise,userTitle:state.settings.title,
...
<body>
  ...
  <main class="login">
    ...
    <div class="secure"><b>●</b> SINGLE-USER SECURE SESSION · BUILD 1.13.3</div>
  </main>
  <script>
    (function(){
...
async function knowledgeUpdate(request, env) {
  try {
    ...
    if (!env.AI) return json({ error: "The Knowledge Update Agent requires the Cloudflare Workers AI binding." }, 503);
...
async function deepResearch(request, env) {
  try {
    ...
    if (!env.AI) return json({ error: "Deep Research requires the Cloudflare Workers AI binding." }, 503);
...
export default {
  async fetch(request, env = {}, ctx = {}) {
    ...
    if (url.pathname === "/api/health") {
      return json({ service: "JARVIS", status: "online", build: "1.13.3", ai: Boolean(env.AI) });
    }

const TEXT_MODELS = Object.freeze({
  ...
  eco: "@cf/zai-org/glm-4.7-flash",
  fast: "@cf/meta/llama-4-scout-17b-16e-instruct",
  balanced: "@cf/zai-org/glm-4.7-flash",
  max: "@cf/openai/gpt-oss-120b",
...
const KNOWLEDGE_JSON_MODEL = "@cf/zai-org/glm-4.7-flash";
const SPEECH_MODEL = "@cf/openai/whisper-large-v3-turbo";
const TTS_MODEL = "@cf/myshell-ai/melotts";
const BUILD_VERSION = "1.15.1";
const CONTEXT_CHARACTER_BUDGETS = Object.freeze([32_000, 64_000, 120_000]);
const FREE_TEXT_FALLBACKS = Object.freeze({
  eco: ["eco", "code", "fast"],
  fast: ["fast", "balanced", "code"],
  balanced: ["balanced", "code", "fast"],
  max: ["max", "balanced", "code"],
  reasoning: ["reasoning", "balanced", "code"],
  code: ["code", "balanced", "fast"],
});
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html,body{height:100%;margin:0}body{background:var(--bg);color:var(--ink);font-family:Inter,"Segoe UI",Arial,sans-serif;overflow:hidden}button,input,textarea,select{color:inherit;font:inherit}button{cursor:pointer}button:focus-visible,input:focus-visible,textarea:focus-visible,select:focus-visible{outline:1px solid var(--cyan);outline-offset:2px}::selection{background:#225d69;color:#fff}::-webkit-scrollbar{width:5px;height:5px}::-webkit-scrollbar-thumb{background:#184450;border-radius:8px}
    ...
    .wake-status{align-items:center;border:1px solid #5ce9ff28;color:#65838b;display:flex;font:600 6px/1 Consolas,monospace;gap:5px;letter-spacing:.1em;margin-left:auto;padding:4px 6px}.wake-status[hidden]{display:none}.wake-status i{background:#536a71;border-radius:50%;height:4px;width:4px}.wake-status.armed{border-color:#5ef6b23d;color:var(--green)}.wake-status.armed i{animation:pulse 2s ease-in-out infinite;background:var(--green);box-shadow:0 0 7px #5ef6b2}.wake-status.awake{border-color:#dfa05a66;color:var(--amber)}.wake-status.awake i{animation:mic .8s ease-in-out infinite;background:var(--amber);box-shadow:0 0 8px #dfa05a}.wake-status.error{border-color:#ff657755;color:#ff8795}.wake-status.error i{background:#ff6577}
    ...
    .settings.help-center{display:flex;flex-direction:column;height:calc(100dvh - 36px);max-width:900px;overflow:hidden}.help-center .settings-head{flex:0 0 auto;position:static}.help-toolbar{align-items:center;background:#071318;display:flex;flex:0 0 auto;gap:7px;padding:13px 0;position:static;z-index:3}.help-scroll{min-height:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;padding-right:4px;scrollbar-gutter:stable;touch-action:pan-y;-webkit-overflow-scrolling:touch}.help-search{align-items:center;border:1px solid #5ce9ff2b;display:flex;flex:1;min-width:0}.help-search span{color:var(--cyan2);padding:0 9px}.help-search input{background:#020b0f;border:0;color:#bdd4d9;font-size:10px;height:36px;min-width:0;outline:0;width:100%}.help-toolbar button{background:#5ce9ff0d;border:1px solid #5ce9ff32;color:#77aeb8;font:600 7px/1 Consolas,monospace;letter-spacing:.08em;min-height:36px;padding:7px 10px}.help-toolbar button:hover{border-color:#5ce9ff70;color:var(--cyan)}.help-summary{background:#5ce9ff06;border:1px solid #5ce9ff18;color:#66858d;font-size:9px;line-height:1.55;margin-bottom:12px;padding:10px}.help-summary strong{color:var(--cyan)}.help-content{display:grid;gap:12px}.help-section{background:#020b0f;border:1px solid #5ce9ff22;padding:12px}.help-section>h3{color:var(--cyan);font:600 9px/1.4 Consolas,monospace;letter-spacing:.12em;margin:0 0 9px}.help-items{display:grid;gap:7px;grid-template-columns:repeat(2,minmax(0,1fr))}.help-item{background:#07171c;border-left:2px solid #1d6673;min-width:0;padding:9px}.help-item h4{color:#a9c2c7;font:600 9px/1.4 Consolas,monospace;margin:0}.help-item p{color:#5f7b83;font-size:8px;line-height:1.55;margin:5px 0 0;white-space:pre-line}.help-item code{color:var(--cyan2);display:block;font:500 7px/1.45 Consolas,monospace;margin-top:6px;overflow-wrap:anywhere}.help-run{background:none;border:1px solid #5ce9ff22;color:#6c9ba4;font:600 6px/1 Consolas,monospace;margin-top:7px;padding:6px}.help-run:hover{border-color:#5ce9ff66;color:var(--cyan)}.help-empty{border:1px dashed #5ce9ff2b;color:#607b83;font-size:9px;padding:24px;text-align:center}@media(max-width:650px){.help-toolbar{align-items:stretch;flex-wrap:wrap}.help-search{flex-basis:100%}.help-toolbar button{flex:1}.help-items{grid-template-columns:1fr}}@media(max-width:560px){.top-actions button:nth-child(3){display:block}.top-actions button:nth-child(4),.top-actions button:nth-child(5){display:none}.settings.help-center{height:calc(100dvh - 16px)}}
    .settings.knowledge-center{display:flex;flex-direction:column;height:calc(100dvh - 36px);max-width:820px;overflow:hidden}.knowledge-center .settings-head{flex:0 0 auto;position:static}.knowledge-form{display:flex;flex:0 0 auto;gap:8px;padding:13px 0}.knowledge-form input{background:#020b0f;border:1px solid #5ce9ff2b;color:#bdd4d9;flex:1;font-size:10px;min-width:0;padding:10px}.knowledge-form button,.knowledge-actions button{background:#5ce9ff0d;border:1px solid #5ce9ff38;color:#78b1bb;font:600 7px/1.2 Consolas,monospace;letter-spacing:.08em;padding:8px 11px}.knowledge-form button:hover,.knowledge-actions button:hover{border-color:#5ce9ff78;color:var(--cyan)}.knowledge-form button:disabled,.knowledge-actions button:disabled{cursor:not-allowed;opacity:.35}.knowledge-status{background:#5ce9ff06;border:1px solid #5ce9ff18;color:#69868d;flex:0 0 auto;font-size:9px;line-height:1.5;margin:0 0 10px;padding:9px}.knowledge-list{display:grid;gap:8px;min-height:0;overflow-x:hidden;overflow-y:auto;padding-right:4px;scrollbar-gutter:stable}.knowledge-empty{border:1px dashed #5ce9ff2b;color:#5e7981;font-size:9px;line-height:1.6;padding:24px;text-align:center}.knowledge-card{background:#020b0f;border:1px solid #5ce9ff25;display:grid;gap:7px;grid-template-columns:auto 1fr;padding:11px}.knowledge-card>input{accent-color:var(--cyan);margin-top:3px}.knowledge-card-body{min-width:0}.knowledge-card-head{align-items:center;display:flex;gap:7px;justify-content:space-between}.knowledge-card-head strong{color:#b4cbd0;font-size:10px;font-weight:500;line-height:1.5}.knowledge-confidence{border:1px solid #dfa05a44;color:var(--amber);flex:0 0 auto;font:600 6px/1 Consolas,monospace;letter-spacing:.08em;padding:4px}.knowledge-confidence.high{border-color:#5ef6b244;color:var(--green)}.knowledge-reason{color:#5b767e;font-size:8px;line-height:1.5;margin:6px 0}.knowledge-sources{display:flex;flex-wrap:wrap;gap:5px}.knowledge-sources a{border:1px solid #5ce9ff20;color:var(--cyan2);font:500 7px/1.3 Consolas,monospace;max-width:100%;overflow:hidden;padding:5px 6px;text-decoration:none;text-overflow:ellipsis;white-space:nowrap}.knowledge-actions{border-top:1px solid var(--line);display:flex;flex:0 0 auto;gap:7px;justify-content:flex-end;margin-top:10px;padding-top:10px}.knowledge-actions .approve{border-color:#5ef6b255;color:var(--green)}@media(max-width:560px){.settings.knowledge-center{height:calc(100dvh - 16px)}.knowledge-form{flex-direction:column}.knowledge-actions{flex-wrap:wrap}.knowledge-actions button{flex:1}.knowledge-card-head{align-items:flex-start;flex-direction:column}}
    .switch-control{align-items:center;display:flex;flex:0 0 auto;gap:7px}.toggle-state{color:#607a82;font:600 7px/1 Consolas,monospace;letter-spacing:.12em;min-width:48px;text-align:right}.toggle-state.on{color:var(--green)}.wake-setting.pending .toggle-state{color:var(--amber)}.wake-setting.pending i{border-color:#dfa05a66}.wake-setting.pending i:after{animation:pulse 1s ease-in-out infinite}
    .modal{overflow-y:auto;overscroll-behavior:contain}.settings{max-height:calc(100vh - 36px);max-height:calc(100dvh - 36px);min-height:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable;touch-action:pan-y;-webkit-overflow-scrolling:touch}.settings-head{background:#071318;box-shadow:0 1px 0 var(--line);isolation:isolate;position:sticky;top:0;z-index:10}
    ...
    @media(max-width:560px){.top{flex-basis:55px;padding:0 9px}.diamond{height:26px;width:26px}.am-copy small{display:none}.top-actions button:nth-child(3),.top-actions button:nth-child(4){display:none}.avatar{margin-left:0}.welcome{justify-content:flex-start;padding:31px 14px var(--composer-inset)}.reactor-stage{margin:-12px 0 -5px;transform:scale(.82)}.welcome h1{font-size:29px}.welcome>p.copy{font-size:11px;margin-bottom:17px;max-width:315px}.cards{gap:6px;grid-template-columns:1fr}.card{min-height:51px}.stream{padding:19px 11px calc(var(--composer-inset) + 18px)}.message{gap:7px}.msg-avatar{flex-basis:31px;height:31px}.msg-avatar .mini{transform:scale(.84)}.msg-col{max-width:calc(100% - 38px)}.bubble{font-size:11px;padding:11px}.composer-zone{padding:33px 7px 7px}.composer{padding:8px}.composer textarea{font-size:11px}.hint{display:none}.send span{display:none}.disclaimer{font-size:6px}.settings{max-height:92dvh;overflow:auto;padding:0 16px 16px}.settings-head{padding-top:16px}.setting{align-items:flex-start;flex-wrap:wrap;padding:12px 0}.setting input[type=range]{margin-left:auto;width:48%}.setting select,.setting input[type=text],.setting input[type=url]{margin-left:auto;max-width:100%}.agent-matrix{grid-template-columns:1fr}.matrix-arrow{transform:rotate(90deg)}.matrix-tools{grid-column:1}.matrix-note{grid-column:1}.skill-grid{grid-template-columns:1fr}.mission-create{flex-direction:column}.mission-step{grid-template-columns:18px 1fr}.mission-step-actions{grid-column:2;grid-row:auto}}
...
<body>
    ...
    <section class="workspace">
      ...
      <div class="body">
        <section class="conversation">
          ...
          <div class="composer-zone"><div class="attachments" id="attachments"></div><div class="composer" id="composer"><div class="compose-status"><span class="pulse"></span><span id="composeStatus">JARVIS ONLINE</span><span class="wake-status" id="wakeStatus" hidden><i></i><b>HEY JARVIS OFF</b></span></div><textarea id="input" rows="1" aria-label="Message JARVIS" placeholder="Ask JARVIS anything…"></textarea><div class="compose-actions"><div><input id="files" type="file" hidden multiple><button id="attach" aria-label="Attach files for safe analysis">＋</button><span class="hint">ANY FILE · SAFE ANALYSIS ONLY · ENTER TO SEND</span></div><div><button class="mic" id="mic" aria-label="Speak to JARVIS">◖</button><button class="send" id="send" disabled><span>SEND</span>↗</button></div></div></div><p class="disclaimer">JARVIS can make mistakes. Uploaded code is analyzed as data and is never executed. Verify important information and code before use.</p></div>
        </section>
        <aside class="telemetry"><div class="tele-head"><span>SYSTEM TELEMETRY</span><i>LIVE</i></div><div class="radar-area"><div class="radar"><span></span><i></i><b></b></div><small>NEURAL MATRIX</small><strong>ONLINE</strong></div><div class="metrics"><div><span>MODEL</span><strong id="modelName">FREE-MAX DIRECTOR</strong></div><div><span>CONTEXT</span><strong id="contextCount">LOCAL</strong></div><div><span>AGENT MATRIX</span><strong class="on">MULTIMODAL</strong></div><div><span>LEARNING</span><strong class="on">KNOWLEDGE RAG</strong></div><div><span>REFLECTION</span><strong class="on">ADAPTIVE CRITIC</strong></div><div><span>CLOUD SYNC</span><strong id="syncState">STANDBY</strong></div><div><span>VOICE LINK</span><strong class="on" id="voiceStatus">ACTIVE</strong></div><div><span>ENCRYPTION</span><strong>AES-GCM</strong></div></div><div class="quota"><div class="quota-top"><span>FREE AI ALLOCATION</span><b>10K</b></div><div class="track"><i></i></div><p>Neurons reset daily at 00:00 UTC. Actual usage is available in Cloudflare.</p></div><div class="commands"><span>SMART COMMANDS</span><button data-command="/aicheck">/aicheck <i>↗</i></button><button data-command="/learn Windows 11 updates">/learn <i>↗</i></button><button data-command="/weather">/weather <i>↗</i></button><button data-command="/settings bluetooth">/settings bluetooth <i>↗</i></button><button data-command="/controlpanel">/controlpanel <i>↗</i></button><button data-command="/app notepad">/app notepad <i>↗</i></button><button data-command="/tools">/tools <i>↗</i></button><button data-command="/folders">/folders <i>↗</i></button><button data-command="/diagnostics">/diagnostics <i>↗</i></button><button data-command="/pc">/pc <i>↗</i></button></div><div class="privacy"><b>◈</b><span><strong>APPROVED ENCRYPTED KNOWLEDGE</strong>Internet findings remain proposals until you approve them. Approved memories can follow your login across devices.</span></div></aside>
      </div>
    </section>
  </main>
  <div class="modal" id="modal">
    <section class="settings" role="dialog" aria-modal="true" aria-label="JARVIS settings">
      ...
      <div class="memory-actions"><button id="maxBrain" type="button">ACTIVATE FREE-MAX UNIFIED BRAIN</button><button id="brainCheck" type="button">RUN COMPLETE JARVIS SELF-CHECK</button><small>Free-Max selects automatic current-model routing, maximum reasoning, globally budgeted 120K-character context, adaptive self-correction, research, memory/RAG, sync, every Smart Skill, and automatic voice. Hey JARVIS stays separate because always-on microphone access requires your explicit choice.</small></div>
      ...
      <label class="setting"><span class="setting-copy"><strong>Model routing</strong><small>Natural chat/code/image routing works with every provider. Free-Max Director uses only built-in free-allocation-compatible Cloudflare routes and safely falls back between them when a model is temporarily unavailable.</small></span><select id="cloudModel"><option value="auto">Free-Max Director — task routing</option><option value="eco">GLM 4.7 Flash — Efficient</option><option value="fast">Llama 4 Scout — Fast / Vision</option><option value="balanced">GLM 4.7 Flash — Balanced</option><option value="max">GPT-OSS 120B — Strong</option><option value="reasoning">GPT-OSS 120B — Reasoning</option><option value="code">Qwen 3 30B — Coding</option></select></label>
      ...
      <label class="setting"><span class="setting-copy"><strong>Managed context budget</strong><small>A global character budget shared safely by chat history, attachments, Memory Vault, semantic RAG, and web evidence. The selected model's real token limit still applies.</small></span><select id="contextWindow"><option value="32000">32K characters — efficient</option><option value="64000">64K characters — extended</option><option value="120000">120K characters — Free-Max</option></select></label>
      ...
      <div class="memory-actions"><input id="chatgptImport" type="file" accept=".json,application/json" hidden multiple><button id="knowledgeAgent" type="button">OPEN KNOWLEDGE UPDATE AGENT</button><button id="importChatGPT" type="button">IMPORT CHATGPT EXPORT</button><button id="clearMemory" type="button">CLEAR MEMORY VAULT</button><small id="memoryCount">Memory Vault is ready. Internet findings require your approval before they become searchable memory.</small></div>
      ...
      <label class="setting toggle"><span class="setting-copy"><strong>Adaptive reflection and self-correction</strong><small>Generator → critic → revision runs for code, complex analysis, research, and higher-risk requests; trivial chat avoids wasting the daily free allocation.</small></span><input id="reflectionMode" type="checkbox" checked><i></i></label>
      ...
      <div class="agent-matrix" aria-label="JARVIS multimodal agent architecture"><div class="matrix-node"><strong>VOICE / VISION</strong>text · speech · images</div><span class="matrix-arrow">→</span><div class="matrix-node"><strong>ORCHESTRATOR LLM</strong>generate · criticize · revise</div><span class="matrix-arrow">→</span><div class="matrix-node"><strong>APPROVED MEMORY RAG</strong>IndexedDB · D1 · vectors</div><div class="matrix-node matrix-tools"><strong>APPROVED TOOL EXECUTION</strong>knowledge research · weather · web · Settings · Control Panel · apps · Windows tools · folders · diagnostics · confirmed PC/IoT actions</div><p class="matrix-note">Internet findings cannot teach JARVIS silently. The Knowledge Update Agent requires two source domains, critic validation, and your explicit approval before saving.</p></div>
      ...
      <div class="skill-grid">
        ...
        <label class="skill toggle"><span class="setting-copy"><strong>Knowledge Update Agent</strong><small>Research, cross-check, and propose knowledge for your approval.</small></span><input id="knowledgeAgentEnabled" type="checkbox" checked><i></i></label>
      </div>
      ...
      <label class="setting toggle"><span class="setting-copy"><strong>Cloud neural voice</strong><small>Use Cloudflare neural speech when available, with the Windows/browser voice as a private fallback.</small></span><input id="neuralVoice" type="checkbox"><i></i></label>
      <label class="setting toggle wake-setting"><span class="setting-copy"><strong>Always listen for “Hey JARVIS”</strong><small>The Windows EXE/MSI continues listening from its visible system-tray icon when minimized and comes forward after the wake phrase. Edge/Chrome listening pauses while the page is hidden and may use an online speech service.</small></span><span class="switch-control"><span class="toggle-state" id="wakeToggleState">OFF</span><input id="wakeWordEnabled" type="checkbox" role="switch" aria-label="Always listen for Hey JARVIS" aria-checked="false"><i aria-hidden="true"></i></span></label>
  ...
  <div class="modal" id="knowledgeModal">
    <section class="settings knowledge-center" role="dialog" aria-modal="true" aria-label="JARVIS Knowledge Update Agent">
      <div class="settings-head"><div><small>RESEARCH · CRITIC · HUMAN APPROVAL</small><h2>KNOWLEDGE UPDATE AGENT</h2></div><button id="closeKnowledge" aria-label="Close Knowledge Update Agent">×</button></div>
      <div class="knowledge-form"><input id="knowledgeTopic" type="text" maxlength="300" placeholder="Topic to research and verify…"><button id="researchKnowledge" type="button">RESEARCH TOPIC</button></div>
      <p class="knowledge-status" id="knowledgeStatus">Enter a topic. JARVIS will compare independent sources and prepare proposals; nothing is saved automatically.</p>
      <div class="knowledge-list" id="knowledgeList"><div class="knowledge-empty">No research draft is open. Try: Windows 11 security updates, Cloudflare Workers AI, or another topic.</div></div>
      <div class="knowledge-actions"><button id="selectKnowledge" type="button" disabled>SELECT ALL</button><button id="rejectKnowledge" type="button" disabled>REJECT DRAFT</button><button class="approve" id="approveKnowledge" type="button" disabled>APPROVE SELECTED</button></div>
    </section>
  </div>
  ...
  <div class="modal" id="helpModal">
    <section class="settings help-center" role="dialog" aria-modal="true" aria-label="JARVIS Help Center">
      ...
      <div class="help-scroll" id="helpScroll">
        <div class="help-summary"><strong>COMPLETE OFFLINE GUIDE · VERSION 1.15.1</strong><br>The Free-Max Unified Brain uses current Cloudflare chat, reasoning, code, and vision routes, safe fallback, global context budgeting, adaptive reflection, and an account-bound route-matrix self-check with sanitized Cloudflare diagnostics. Optional local Ollama and every existing command, skill, voice, memory, research, file, and Windows function remain available.</div>
  ...
  <script>
    (function(){
      ...
      var modelLabels={auto:"FREE-MAX DIRECTOR",eco:"GLM 4.7 · FLASH",fast:"LLAMA 4 · SCOUT",balanced:"GLM 4.7 · FLASH",max:"GPT-OSS · 120B",reasoning:"GPT-OSS · 120B",code:"QWEN 3 · 30B"};
      ...
      var HELP_SECTIONS=[
        {title:"GETTING STARTED TUTORIALS",items:[
          ...
          {name:"Test Unified JARVIS Brain",description:"1. Open Settings → AI Core and select Unified JARVIS Brain.\n2. Select ACTIVATE FREE-MAX UNIFIED BRAIN; the configuration saves automatically.\n3. Select RUN COMPLETE JARVIS SELF-CHECK or type /aicheck. JARVIS tests the account-bound general, code, reasoning, and vision-text routes separately, then reports the exact sanitized error classification and Cloudflare code for any failed route.\n4. The check also identifies sync, RAG, research, document conversion, Windows-link, and updater configuration.\n5. READY means every route passed. DEGRADED with at least one ready route means JARVIS can continue through fallback. NOT REACHED means Cloudflare rejected inference before a model responded."},
          ...
          {name:"Save this complete guide",description:"Select SAVE GUIDE (.MD) at the top. JARVIS stores a local offline copy and downloads JARVIS-Help-Guide-1.15.1.md. Open it in Notepad, VS Code, Word, or any Markdown reader."}
        ]},
        {title:"AI, CHAT, AND VOICE COMMANDS",items:[
          ...
          {name:"/aicheck",description:"Run authenticated account-bound probes for the general, code, reasoning, and vision-text routes. The report shows each route's model, latency, sanitized Cloudflare error classification/code, plus document conversion, encrypted sync, semantic RAG, SearXNG research, Windows link, updater, context budget, and free-route policy. The public Playground is not used.",example:"/aicheck"},
          {name:"/new",description:"Start a new transmission without removing the New Conversation function.",example:"/new"},
          {name:"/clear",description:"Clear the messages in the current transmission. This does not remove the New Conversation function, other saved conversations, or Memory Vault.",example:"/clear"},
          ...
          {name:"Hey JARVIS wake phrase",description:"When enabled in Settings, say ‘Hey Jarvis’ to receive a spoken acknowledgement and dictate a request. You can also say ‘Hey Jarvis, what is the weather?’ in one sentence. The visible armed indicator confirms when the microphone listener is active.",example:"Hey Jarvis, open Notepad"},
        ...
        {title:"ADVANCED INTELLIGENCE AND MULTIMODAL COMMANDS",items:[
          {name:"Version 1.14.0 unified provider cleanup",description:"JARVIS uses built-in Cloudflare AI with optional local Ollama fallback. Vision, voice, wake phrase, memory/RAG, sync, research, file analysis, Mission Control, Windows tools, updater, natural commands, and every slash command remain available."},
          {name:"Version 1.15.1 Account-Bound Brain Diagnostics",description:"JARVIS directs general work to GLM 4.7 Flash, maximum reasoning to GPT-OSS 120B, coding to Qwen3 30B, and image/screen analysis to Llama 4 Scout. The new route matrix tests all four through the deployed Worker binding, distinguishes allocation, rate-limit, billing, access, timeout, model, and unknown failures, and safely displays the real Cloudflare code. Model-specific access errors may use another built-in free route; account-wide allocation, rate, and billing errors stop immediately."},
          ...
          {name:"Unified JARVIS Brain",description:"Select Unified JARVIS Brain or press ACTIVATE FREE-MAX UNIFIED BRAIN. Natural requests are classified as chat, code, image, research, file learning, screen vision, or protected tools. Free-Max Director chooses the current built-in Cloudflare general, code, reasoning, and vision routes and can retry a different built-in free route only for a temporary model failure. In the genuine Windows app, Unified mode can then try configured local Ollama for supported text work. Manual Cloudflare-only and Ollama-only overrides remain under Advanced AI Overrides."},
          {name:"Built-in Cloudflare AI",description:"No third-party API key is needed. Cloudflare Workers AI powers GLM 4.7 Flash general chat, GPT-OSS 120B reasoning, Qwen3 30B coding, Llama 4 Scout vision, Deep Research, Knowledge Update Generator/Critic, adaptive Generator → Critic → Revision, BGE semantic embeddings, Whisper transcription, MeloTTS voice, and FLUX image synthesis. Only free-allocation-compatible routes are enabled; paid-only model routes are excluded."},
          {name:"Response progress, errors, and recovery",description:"While a request is active, JARVIS displays provider selection, generation, review, and finalization stages and automatically follows the latest response. Encrypted sync waits until generation finishes, and the reply is written to the current saved conversation even if background state changes. A timeout, quota problem, rejected request, or empty model result becomes a visible assistant message and can also be spoken. Only a request carrying a real pending marker at startup receives an interrupted-request notice."},
          ...
          {name:"Globally budgeted context",description:"Choose 32K, 64K, or 120K managed characters in Settings. The limit is shared across conversation history, attachments, local memory, synchronized semantic RAG, web evidence, and system instructions instead of being applied independently to each source. Coding routes clamp safely for their smaller model context. Uploaded content remains untrusted reference material."},
          ...
          {name:"Photoshop PSD analysis",description:"In the updated EXE/MSI, attach a standard PSD up to 20 MB. JARVIS safely extracts canvas properties, layer hierarchy, names, visibility, opacity, blend modes, bounds, masks, Smart Object indicators, and available text-layer content. It skips pixel data and linked-file payloads and never runs actions, macros, or scripts. PSB and some newer Photoshop features may require export."},
          {name:"Microsoft Office learning",description:"The updated EXE/MSI safely parses modern Word, Excel, and PowerPoint Open XML files up to 30 MB. Word content, Excel sheets/cells/formulas/results/named ranges, and PowerPoint slides/notes are extracted. Macro-enabled files are detected, but VBA, external links, embedded programs, and package binaries are never executed. Use /learnfiles to approve RAG storage."},
          {name:"Maximum local AI",description:"Run SETUP_MAX_LOCAL_AI.bat. It detects installed RAM, recommends and downloads practical Ollama chat, coding, vision, and embedding models only after confirmation, and configures Flash Attention plus a large local context. Restart Ollama and JARVIS afterward."},
          {name:"Local processing limits",description:"Local AI is private and has no Cloudflare neuron quota, but speed and model size depend on RAM, GPU VRAM, storage, and cooling. The adaptive setup does not pretend a model larger than the computer can run reliably."}
        ]},
        ...
        {title:"CORE AI AND LEARNING FUNCTIONS",items:[
          ...
          {name:"Attachments and vision input",description:"Attach text, source code, SQL, structured data, office documents, images, audio, or other files for safe analysis. JARVIS never executes uploaded code, scripts, macros, installers, or database commands."},
          ...
          {name:"Natural requests plus slash commands",description:"Normal language is the recommended interface: ask JARVIS to write code, open an app, research a topic, analyze files, or check Windows. The Intent Director selects the matching skill. Slash commands remain available for precise, repeatable, and diagnostic use; they were not removed."},
          {name:"Adaptive reflection and self-correction",description:"Generator → critic → revision runs for code, complex analysis, research, and higher-risk requests. Simple greetings and trivial chat use one model call so the free daily allocation is not wasted. Uncertain reviewed answers are routed to human validation."},
        ...
        {title:"APPLICATION, SECURITY, AND UPDATE FUNCTIONS",items:[
          ...
          {name:"Function-retention policy",description:"Updates are additive unless a function is unsafe, retired by its provider, broken, or explicitly requested for removal. Version 1.15.1 keeps the Free-Max routes and adds exact account-bound route diagnostics while preserving every command, smart skill, voice feature, memory function, file analyzer, desktop tool, updater, natural-language route, slash command, and safety boundary. Help Center entries are tested against their command and feature markers before release."},
      ...
      function recoverInterruptedReplies(){var changed=false;state.conversations.forEach(function(c){var messages=Array.isArray(c.messages)?c.messages:[],last=messages[messages.length-1];if(last&&last.role==="user"&&last.requestPending===true){last.requestPending=false;messages.push({id:id("msg"),role:"assistant",content:"That request was interrupted when JARVIS closed or reloaded before the response completed. Please send it once more.",createdAt:Date.now()});c.updatedAt=Date.now();changed=true}});return changed}
      ...
      function syncSettings(){return{cloudModel:state.settings.cloudModel,reasoningPower:state.settings.reasoningPower,contextWindow:Number(state.settings.contextWindow),webSearch:!!state.settings.webSearch,memoryEnabled:!!state.settings.memoryEnabled,reflectionMode:state.settings.reflectionMode!==false,missionControlEnabled:state.settings.missionControlEnabled!==false,screenVisionEnabled:state.settings.screenVisionEnabled!==false,itCopilotEnabled:state.settings.itCopilotEnabled!==false,proactiveBriefingEnabled:state.settings.proactiveBriefingEnabled!==false,knowledgeAgentEnabled:state.settings.knowledgeAgentEnabled!==false,autoSpeak:!!state.settings.autoSpeak,neuralVoice:!!state.settings.neuralVoice,concise:!!state.settings.concise,creativity:Number(state.settings.creativity),title:String(state.settings.title||""),weatherLocation:String(state.settings.weatherLocation||"Iloilo City, Philippines").slice(0,120)}}
      ...
      function cleanConversation(c){return{id:String(c.id||id("chat")),title:String(c.title||"New transmission").slice(0,200),mode:c.mode==="code"||c.mode==="image"?c.mode:"chat",createdAt:Number(c.createdAt||Date.now()),updatedAt:Number(c.updatedAt||Date.now()),messages:(Array.isArray(c.messages)?c.messages:[]).map(function(m){return{id:String(m.id||id("msg")),role:m.role==="assistant"?"assistant":"user",content:String(m.content||"").slice(0,24000),image:typeof m.image==="string"?m.image.slice(0,4000000):"",createdAt:Number(m.createdAt||Date.now()),feedback:m.feedback==="helpful"||m.feedback==="corrected"?m.feedback:"",reviewNeeded:!!m.reviewNeeded,reviewed:!!m.reviewed,requestPending:m.requestPending===true,mission:cleanMission(m.mission)}})}}
      ...
      function scheduleSync(delay){if(state.syncApplying||state.syncing||!state.settings.cloudSync)return;clearTimeout(state.syncTimer);state.syncTimer=setTimeout(function(){if(state.sending){scheduleSync(1200);return}syncNow(false,false)},typeof delay==="number"?delay:2500)}
      async function syncNow(forceLocal,notifyUser){if(state.syncing)return;if(state.sending){scheduleSync(1200);if(notifyUser)toast("Synchronization will resume after JARVIS finishes this response.");return}if(!state.settings.cloudSync&&!forceLocal){setSyncStatus("OFF",false,"Cloud synchronization is disabled on this device.");return}state.syncing=true;setSyncStatus("SYNCING",false,"Securely merging this device with your Cloudflare snapshot…");try{var remote=await syncFetch("GET"),merged,upload,result;if(state.sending){scheduleSync(1200);return}for(var attempt=0;attempt<2;attempt++){var local=await buildLocalSyncData();if(state.sending){scheduleSync(1200);return}merged=forceLocal?local:mergeSyncData(local,remote.data);if(!forceLocal)await applySyncData(merged);if(state.sending){scheduleSync(1200);return}upload=fitSyncData(merged);var unchanged=remote.data&&JSON.stringify(upload)===JSON.stringify(normalizeSyncData(remote.data));if(unchanged){result={revision:remote.revision,updatedAt:remote.updatedAt};break}try{result=await syncFetch("PUT",{baseRevision:Number(remote.revision||0),deviceId:state.syncMeta.deviceId,data:upload});break}catch(error){if(error.status!==409||attempt===1)throw error;remote=await syncFetch("GET");forceLocal=false;if(state.sending){scheduleSync(1200);return}}}state.syncMeta.revision=Number(result.revision||remote.revision||0);saveLocalOnly();var time=new Date(Number(result.updatedAt||Date.now())).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),note=upload.truncated?" Synced the newest data within the encrypted snapshot limit; older local items remain on this device.":"";setSyncStatus("SYNCED",true,"Last synchronized at "+time+"."+note);if(notifyUser)toast("All devices synchronized.")}catch(error){var setup=error.status===503;setSyncStatus(setup?"SETUP NEEDED":navigator.onLine?"RETRYING":"OFFLINE",false,error.message);if(notifyUser)toast(error.message)}finally{state.syncing=false}}
      ...
      function renderKnowledgeDraft(){var draft=state.knowledgeDraft,list=q("#knowledgeList"),status=q("#knowledgeStatus"),proposals=draft&&Array.isArray(draft.proposals)?draft.proposals:[];if(!draft){status.textContent="Enter a topic. JARVIS will compare independent sources and prepare proposals; nothing is saved automatically.";list.innerHTML='<div class="knowledge-empty">No research draft is open. Every update requires two source domains, critic validation, and your approval.</div>'}else{status.textContent=draft.summary||"Review each proposal and its sources. Nothing has been saved.";list.innerHTML=proposals.length?proposals.map(function(proposal,index){var sources=Array.isArray(proposal.sources)?proposal.sources:[];return'<label class="knowledge-card"><input class="knowledge-choice" type="checkbox" data-index="'+index+'"><span class="knowledge-card-body"><span class="knowledge-card-head"><strong>'+esc(proposal.fact)+'</strong><span class="knowledge-confidence '+(proposal.confidence==="high"?"high":"")+'">'+esc(String(proposal.confidence||"medium").toUpperCase())+'</span></span><p class="knowledge-reason">'+esc(proposal.reason||"Cross-checked by the Knowledge Update Critic.")+'</p><span class="knowledge-sources">'+sources.map(function(source){return'<a href="'+esc(source.url)+'" target="_blank" rel="noopener noreferrer" title="'+esc(source.url)+'">'+esc(source.title||source.url)+'</a>'}).join("")+'</span></span></label>'}).join(""):'<div class="knowledge-empty">No proposed fact passed both the independent-source rule and the critic review. Nothing can be approved or saved.</div>'}q("#selectKnowledge").disabled=!proposals.length;q("#rejectKnowledge").disabled=!draft;q("#approveKnowledge").disabled=!proposals.length;q("#selectKnowledge").textContent="SELECT ALL"}
      function openKnowledgeAgent(topic){if(!state.settings.knowledgeAgentEnabled){toast("Knowledge Update Agent is disabled in Settings.");return}closeHelpCenter();closeMissionControl();closeSettings();q("#knowledgeModal").classList.add("open");if(typeof topic==="string")q("#knowledgeTopic").value=topic.slice(0,300);renderKnowledgeDraft();requestAnimationFrame(function(){q("#knowledgeTopic").focus()})}
      function closeKnowledgeAgent(){q("#knowledgeModal").classList.remove("open")}
      function appendKnowledgeResult(draft,text){if(!draft||!draft.ownerId)return;var owner=state.conversations.find(function(conversation){return conversation.id===draft.ownerId});if(!owner)return;owner.messages.push({id:id("msg"),role:"assistant",content:text,createdAt:Date.now()});owner.updatedAt=Date.now();save();render()}
      async function createKnowledgeUpdate(topic,displayPrompt){topic=String(topic||"").replace(/\s+/g," ").trim().slice(0,300);if(topic.length<3){toast("Enter a knowledge topic of at least three characters.");return}if(!state.settings.knowledgeAgentEnabled){toast("Knowledge Update Agent is disabled in Settings.");return}if(!state.settings.memoryEnabled){toast("Enable Learn from my conversations before updating the Memory Vault.");return}var c=current(),draftMeta={ownerId:c.id,fromChat:!!displayPrompt};openKnowledgeAgent(topic);q("#knowledgeStatus").textContent="RESEARCHING: searching twice, comparing source domains, generating claims, and running the critic…";q("#knowledgeList").innerHTML='<div class="knowledge-empty">Research in progress. No information is being saved.</div>';q("#researchKnowledge").disabled=true;q("#selectKnowledge").disabled=true;q("#rejectKnowledge").disabled=true;q("#approveKnowledge").disabled=true;if(displayPrompt){chatStickToBottom=true;c.messages.push({id:id("msg"),role:"user",content:displayPrompt,createdAt:Date.now()});if(c.messages.length===1)c.title=("Knowledge: "+topic).slice(0,48);c.updatedAt=Date.now();q("#input").value="";state.sending=true;save();render()}try{var data=await postJson("/api/knowledge-update",{topic:topic,provider:state.settings.provider});state.knowledgeDraft=Object.assign(data,draftMeta);renderKnowledgeDraft();if(displayPrompt){appendKnowledgeResult(state.knowledgeDraft,"**KNOWLEDGE REVIEW READY**\n"+(data.summary||"Review the proposed facts and sources in the Knowledge Update Agent.")+"\n\n"+(data.proposals&&data.proposals.length?data.proposals.length+" critic-approved proposal(s) await your decision. Nothing has been saved.":"No claim passed verification. Nothing was saved."));if(data.proposals&&data.proposals.length&&state.settings.autoSpeak)speak("Knowledge review ready, "+(state.settings.title||"sir")+". Nothing will be saved without your approval.")}}catch(error){state.knowledgeDraft=null;renderKnowledgeDraft();q("#knowledgeStatus").textContent=error.message;if(displayPrompt)appendKnowledgeResult(draftMeta,"The Knowledge Update Agent could not complete its research. "+error.message)}finally{state.sending=false;q("#researchKnowledge").disabled=false;save();render()}}
      ...
      var recognition=null,wakeRecognition=null,wakeRestartTimer=null,wakeUsingNative=false,wakeStarting=false,wakeHandling=false,speechToken=0;
      function desktopWakeApi(){var api=window.jarvisDesktop;return api&&typeof api.setWakeWordEnabled==="function"?api:null}
      function updateWakeToggle(enabled,pending){var input=q("#wakeWordEnabled"),label=q("#wakeToggleState"),setting=document.querySelector(".wake-setting");enabled=enabled===true;pending=pending===true;if(input){input.checked=enabled;input.setAttribute("aria-checked",enabled?"true":"false");input.setAttribute("aria-busy",pending?"true":"false")}if(label){label.textContent=pending?"STARTING":enabled?"ON":"OFF";label.classList.toggle("on",enabled&&!pending)}if(setting)setting.classList.toggle("pending",pending)}
      function disableWakeWord(message,notice){state.settings.wakeWordEnabled=false;wakeStarting=false;wakeUsingNative=false;updateWakeToggle(false,false);state.syncMeta.settingsUpdatedAt=Date.now();saveLocalOnly();updateWakeStatus("error",message||"The wake-word listener could not start.");toast(notice||"Hey JARVIS was turned off because the listener could not start.")}
      function updateWakeStatus(status,message){var badge=q("#wakeStatus"),label=badge&&badge.querySelector("b");if(!badge||!label)return;badge.className="wake-status";badge.hidden=status==="off"&&!state.settings.wakeWordEnabled;if(status==="armed"){badge.classList.add("armed");label.textContent="HEY JARVIS ARMED";updateWakeToggle(true,false)}else if(status==="awake"){badge.classList.add("awake");label.textContent="JARVIS AWAKE"}else if(status==="error"||status==="unavailable"){badge.classList.add("error");label.textContent=status==="unavailable"?"WAKE WORD UNAVAILABLE":"WAKE WORD ERROR"}else if(status==="paused"){label.textContent="WAKE WORD PAUSED"}else if(status==="starting"){label.textContent="STARTING WAKE WORD…";updateWakeToggle(true,true)}else{label.textContent="HEY JARVIS OFF";updateWakeToggle(false,false)}if(message)badge.title=message;else badge.removeAttribute("title");q("#voiceStatus").textContent=status==="armed"?"WAKE ARMED":state.settings.autoSpeak?"ACTIVE":"STANDBY";q("#voiceStatus").classList.toggle("on",status==="armed"||state.settings.autoSpeak)}
      async function stopWakeWordListener(preserveSetting){clearTimeout(wakeRestartTimer);wakeRestartTimer=null;wakeStarting=false;var browser=wakeRecognition;wakeRecognition=null;if(browser){browser.onend=null;browser.onerror=null;try{browser.abort()}catch(error){}}var api=desktopWakeApi();if(api){try{await api.setWakeWordEnabled(false)}catch(error){}}wakeUsingNative=false;if(!preserveSetting){state.settings.wakeWordEnabled=false;updateWakeToggle(false,false);state.syncMeta.settingsUpdatedAt=Date.now();saveLocalOnly();updateWakeStatus("off")}}
      function scheduleWakeWord(delay){clearTimeout(wakeRestartTimer);if(!state.settings.wakeWordEnabled)return;wakeRestartTimer=setTimeout(startWakeWordListener,Math.max(100,Number(delay)||0))}
      function wakePhraseResult(text){var match=String(text||"").trim().match(/\bhey\s+jarvis\b[\s,.:;!?-]*(.*)$/i);return match?{heard:true,command:String(match[1]||"").trim()}:null}
      function startBrowserWakeWord(){var R=window.SpeechRecognition||window.webkitSpeechRecognition;if(!R){disableWakeWord("This browser does not provide continuous speech recognition.","Hey JARVIS was turned off. Use the Windows app or current Edge/Chrome.");return false}var instance=new R();wakeRecognition=instance;instance.continuous=true;instance.interimResults=false;instance.lang="en-US";instance.onstart=function(){if(wakeRecognition===instance)updateWakeStatus("armed","Listening only for the Hey JARVIS wake phrase while this app is open.")};instance.onresult=function(event){for(var i=event.resultIndex;i<event.results.length;i++){if(!event.results[i].isFinal)continue;var phrase=wakePhraseResult(event.results[i][0].transcript);if(phrase){handleWakeWord(phrase);break}}};instance.onerror=function(event){if(wakeRecognition!==instance)return;var reason=String(event.error||"");if(reason==="aborted"||reason==="no-speech")return;if(reason==="not-allowed"){disableWakeWord("Microphone access was denied.","Hey JARVIS was turned off because microphone access was denied.");return}if(reason==="service-not-allowed"){disableWakeWord("The browser speech-recognition service is unavailable.","Hey JARVIS was turned off because the browser speech service is unavailable.");return}updateWakeStatus("error","Speech recognition stopped: "+reason)};instance.onend=function(){if(wakeRecognition===instance)wakeRecognition=null;if(state.settings.wakeWordEnabled&&!wakeHandling)scheduleWakeWord(900)};try{instance.start();return true}catch(error){wakeRecognition=null;disableWakeWord(error.message,"Hey JARVIS was turned off because speech recognition could not start.");return false}}
      async function startWakeWordListener(){wakeRestartTimer=null;if(!state.settings.wakeWordEnabled)return false;if(state.speaking||state.listening||state.sending||wakeHandling){scheduleWakeWord(900);return false}if(wakeRecognition||wakeUsingNative||wakeStarting)return true;var api=desktopWakeApi();if(api){wakeStarting=true;updateWakeStatus("starting","Starting the local Windows wake-word listener.");try{var result=await api.setWakeWordEnabled(true);if(!state.settings.wakeWordEnabled){wakeStarting=false;try{await api.setWakeWordEnabled(false)}catch(stopError){}return false}wakeStarting=false;wakeUsingNative=true;updateWakeStatus(result&&result.status==="listening"?"armed":"starting","Windows speech recognition is waiting for Hey JARVIS.");return true}catch(error){disableWakeWord(error.message,"Hey JARVIS was turned off because the Windows listener could not start.");return false}}return startBrowserWakeWord()}
      function syncWakeWordSetting(){if(state.settings.wakeWordEnabled){updateWakeToggle(true,true);scheduleWakeWord(100)}else stopWakeWordListener(true).then(function(){updateWakeStatus("off")})}
      function handleNativeWakeStatus(payload){if(!state.settings.wakeWordEnabled)return;var status=String(payload&&payload.status||""),message=String(payload&&payload.message||"");if(status==="listening"){wakeStarting=false;wakeUsingNative=true;updateWakeStatus("armed","Windows speech recognition is listening locally for Hey JARVIS.");return}if(status==="error"||status==="stopped"){disableWakeWord(message||"Windows wake-word recognition stopped.","Hey JARVIS was turned off. "+(message||"The Windows speech listener stopped."))}}
      function handleWakeWord(result){if(!state.settings.wakeWordEnabled||wakeHandling)return;var phrase=result&&result.heard?result:wakePhraseResult(result&&result.transcript||result);if(!phrase)return;wakeHandling=true;updateWakeStatus("awake");stopWakeWordListener(true);var title=state.settings.title||"sir";if(phrase.command){q("#input").value=phrase.command;inputChanged();speak("Yes, "+title+".",function(){wakeHandling=false;sendV2()});return}speak("At your service, "+title+". How may I assist?",function(){wakeHandling=false;startVoiceInput(true)})}
      function speakSystem(text,onDone){var callback=typeof onDone==="function"?onDone:function(){};if(!("speechSynthesis" in window)){toast("Speech synthesis is not supported by this browser.");callback();scheduleWakeWord(700);return}var token=++speechToken;state.speaking=true;updateWakeStatus(state.settings.wakeWordEnabled?"paused":"off","Wake-word detection pauses while JARVIS speaks.");stopWakeWordListener(true);speechSynthesis.cancel();var u=new SpeechSynthesisUtterance(String(text).replace(/[*#]/g,"").slice(0,8000)),voices=speechSynthesis.getVoices(),names=["Daniel","Arthur","Ryan","Google UK English Male"],v=null;names.some(function(n){v=voices.find(function(x){return x.name.indexOf(n)>-1});return!!v});if(!v)v=voices.find(function(x){return x.lang.toLowerCase().indexOf("en-gb")===0})||voices.find(function(x){return x.lang.toLowerCase().indexOf("en")===0});if(v)u.voice=v;u.lang="en-GB";u.rate=.92;u.pitch=.78;var finished=false,finish=function(){if(finished||token!==speechToken)return;finished=true;state.speaking=false;callback();if(!state.listening&&!wakeHandling)scheduleWakeWord(700)};u.onend=finish;u.onerror=finish;speechSynthesis.speak(u)}
      async function speak(text,onDone){var callback=typeof onDone==="function"?onDone:function(){};if(!state.settings.neuralVoice){speakSystem(text,callback);return}var clean=String(text).replace(/[*#]/g,"").slice(0,1800),token=++speechToken;state.speaking=true;updateWakeStatus(state.settings.wakeWordEnabled?"paused":"off","Wake-word detection pauses while JARVIS speaks.");await stopWakeWordListener(true);try{var response=await fetch("/api/speech",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({text:clean})});if(!response.ok)throw new Error("Neural voice unavailable");var blob=await response.blob(),url=URL.createObjectURL(blob),audio=new Audio(url),finish=function(){URL.revokeObjectURL(url);if(token!==speechToken)return;state.speaking=false;callback();if(!state.listening&&!wakeHandling)scheduleWakeWord(700)};audio.onended=finish;audio.onerror=finish;await audio.play()}catch(error){if(token===speechToken)state.speaking=false;speakSystem(clean,callback)}}
      function startVoiceInput(autoSend){var R=window.SpeechRecognition||window.webkitSpeechRecognition;if(!R){toast("Voice input works best in Chrome or Edge. You can still type below.");scheduleWakeWord(900);return}stopWakeWordListener(true);var instance=new R(),captured="";recognition=instance;instance.continuous=false;instance.interimResults=true;instance.lang="en-US";instance.onresult=function(event){var text="";for(var i=0;i<event.results.length;i++)text+=event.results[i][0].transcript;captured=text.trim();q("#input").value=text;inputChanged()};instance.onend=function(){if(recognition===instance)recognition=null;state.listening=false;micState();if(autoSend&&captured){sendV2();return}scheduleWakeWord(800)};instance.onerror=function(event){if(recognition===instance)recognition=null;state.listening=false;micState();if(String(event.error||"")!=="aborted")toast("Microphone access failed. Check browser permission and retry.");scheduleWakeWord(1000)};state.listening=true;micState();try{instance.start()}catch(error){state.listening=false;micState();toast("Microphone access failed. "+error.message);scheduleWakeWord(1000)}}
      function toggleMic(){if(state.listening){if(recognition)recognition.stop();return}startVoiceInput(false)}
      ...
      async function askOllama(c,context,memory,research){
        ...
        var localMax=state.settings.reasoningPower==="max",localDeep=state.settings.reasoningPower==="deep"||localMax,localMessages=localMax?30:localDeep?24:18,maxTokens=localMax?4096:localDeep?3072:2048;personality+="\n\nReasoning profile: "+state.settings.reasoningPower+". "+(localDeep?"Before answering, privately check requirements, logic, factual uncertainty, and safety. Return only the useful final reasoning and answer; do not expose hidden chain-of-thought.":"");var messages=[{role:"system",content:personality+"\n\n"+job}].concat(c.messages.slice(-localMessages).map(function(m){return{role:m.role,content:m.content}})),localApi=window.jarvisDesktop&&typeof window.jarvisDesktop.askLocalAI==="function"?window.jarvisDesktop:null;if(localApi){var localResult=await localApi.askLocalAI({endpoint:base,model:state.settings.ollamaModel,messages:messages,temperature:state.settings.creativity,maxTokens:maxTokens,contextWindow:Number(state.settings.contextWindow)||64000});return localResult.response}var res=await fetch(base+"/chat/completions",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({model:state.settings.ollamaModel,messages:messages,temperature:state.settings.creativity,max_tokens:maxTokens,stream:false,options:{num_ctx:Number(state.settings.contextWindow)||64000,num_predict:maxTokens}})}),data=await res.json();
      ...
      function beginAiProgress(provider,mode){var timers=[],label=provider==="auto"?"UNIFIED BRAIN · SELECTING FREE ROUTE":provider==="ollama"?"CONTACTING LOCAL OLLAMA":"CONTACTING BUILT-IN CLOUDFLARE AI";state.aiStage=label;timers.push(setTimeout(function(){if(state.sending){state.aiStage=mode==="code"?"GENERATING AND REVIEWING CODE":"GENERATING RESPONSE";renderMessages()}},4500));timers.push(setTimeout(function(){if(state.sending){state.aiStage="FINALIZING RESPONSE";renderMessages()}},18000));return function(){timers.forEach(clearTimeout);state.aiStage=""}}
      var naturalOriginalPrompt="";
      function localReply(prompt,response,speech){var shown=naturalOriginalPrompt||prompt,c=current(),now=Date.now();naturalOriginalPrompt="";chatStickToBottom=true;c.messages.push({id:id("msg"),role:"user",content:shown,createdAt:now});c.messages.push({id:id("msg"),role:"assistant",content:response,createdAt:Date.now()});if(c.messages.length===2)c.title=shown.replace(/\s+/g," ").slice(0,48);c.updatedAt=Date.now();q("#input").value="";save();render();if(state.settings.autoSpeak)speak(typeof speech==="string"?speech:response)}
      function openExternal(url){var opened=null;try{opened=window.open(url,"_blank","noopener,noreferrer")}catch(e){}return opened}
      async function runBrainSelfCheck(prompt){state.sending=true;state.aiStage="TESTING UNIFIED BRAIN ROUTES";render();try{var data=await postJson("/api/brain-check",{}),bindings=data.bindings||{},live=data.live||{},summary=data.summary||{},probes=Array.isArray(data.probes)?data.probes:[],checks=["Cloudflare AI binding: "+(bindings.cloudflareAI?"CONNECTED":"NOT BOUND"),"Account-bound inference: "+(summary.accountInferenceReached?"REACHED":"NOT REACHED")];probes.forEach(function(probe){var detail=probe.ok?"PASS · "+probe.model+" · "+probe.latencyMs+" ms":"FAIL · "+String(probe.classification||"unknown").toUpperCase()+(probe.code?" ["+probe.code+"]":"")+" · "+(probe.message||"No diagnostic returned");checks.push("Brain route "+String(probe.key||"unknown").toUpperCase()+": "+detail)});checks.push("Document conversion: "+(bindings.documentConversion?"READY":"UNAVAILABLE"));checks.push("Encrypted sync: "+(bindings.encryptedSync?"READY":"NOT CONFIGURED"));checks.push("Semantic memory/RAG: "+(bindings.semanticMemory?"READY":"NOT CONFIGURED"));checks.push("SearXNG research: "+(bindings.webResearch?"READY":"NOT CONFIGURED"));var desktopApi=window.jarvisDesktop&&typeof window.jarvisDesktop.getCapabilities==="function"?window.jarvisDesktop:null;if(desktopApi){try{var desktop=await desktopApi.getCapabilities();checks.push("Windows desktop link: READY · "+desktop.desktopVersion);checks.push("Automatic updates: "+(desktop.automaticUpdates?"READY":"NOT CONFIGURED"))}catch(desktopError){checks.push("Windows desktop link: ATTENTION")}}else checks.push("Windows desktop link: BROWSER / PWA MODE");state.sending=false;localReply(prompt,"**JARVIS UNIFIED BRAIN SELF-CHECK — "+String(data.status||"unknown").toUpperCase()+"**\n\n"+checks.map(function(item){return"• "+item}).join("\n")+"\n\nBuild: "+data.build+" · routes ready: "+Number(summary.readyRoutes||0)+"/"+Number(summary.testedRoutes||0)+" · managed context: "+Number(data.context&&data.context.maximumManagedCharacters||0).toLocaleString()+" characters · paid-only AI routes: OFF\n\nThis report uses account-bound Worker inference, not the public Playground. Sanitized Cloudflare route errors are displayed without API keys or authorization data.",String(data.status||"")==="ready"?"All Unified Brain routes passed, "+(state.settings.title||"sir")+".":live.ok?"JARVIS can answer through an available fallback route, but one or more brain routes need attention, "+(state.settings.title||"sir")+".":"Account-bound Cloudflare inference did not start. Review the displayed route error, "+(state.settings.title||"sir")+".")}catch(error){state.sending=false;localReply(prompt,"The JARVIS self-check could not complete. "+error.message,false)}return true}
      ...
      runSmartAction=async function(prompt){var lower=String(prompt||"").toLowerCase().trim();if(lower==="/aicheck"||lower==="/selfcheck"||lower==="run jarvis self check"||lower==="check jarvis brain"||lower==="test jarvis brain"||lower==="is jarvis working")return runBrainSelfCheck(prompt);if(lower==="/help"||lower==="help"||lower.indexOf("what can you do")>-1||lower.indexOf("show commands")>-1){localReply(prompt,"**JARVIS 1.15.1 SMART ACTIONS**\n/aicheck — test every account-bound Unified Brain route and resource\n/mission [goal] — build a reviewable Mission Control plan\n/missions — open Mission Control\n/screen [question] — analyze one approved screen capture\n/itcheck — run a read-only Windows IT health analysis\n/briefing — weather, device, mission, and review briefing\n/skills — show enabled smart skills\n/weather [place] — current conditions and forecast\n/open [site or URL] · /search [query] · /maps [place] · /youtube [query]\n/settings [area] · /controlpanel [item] · /app [name]\n/tools · /tool [name] · /folders · /folder [name]\n/diagnostics · /diagnose [name] · /pc [action]\n/system · /calculate [math] · /speak [text] · /mute · /new · /remember [fact]\n\nNatural language remains available for every supported action, and every slash command remains available. Mission steps never bypass the confirmation required by Windows actions.","Smart actions and the Free-Max Unified Brain are online, "+(state.settings.title||"sir")+".");return true}if(await runSmartUpgradeAction(prompt))return true;return runSmartActionV10(prompt)};
      ...
      var runSmartActionV12=runSmartAction;
      runSmartAction=async function(prompt){var lower=String(prompt||"").toLowerCase().trim(),match;if(lower==="/learn"||lower==="/knowledge"||lower==="update your knowledge"){q("#input").value="";inputChanged();openKnowledgeAgent("");return true}match=String(prompt||"").match(/^\/(?:learn|knowledge)\s+(.+)$/i)||String(prompt||"").match(/^(?:jarvis[,\s]+)?(?:update|refresh)\s+(?:your\s+)?knowledge\s+(?:about|on)\s+(.+)$/i);if(match){await createKnowledgeUpdate(match[1],prompt);return true}return runSmartActionV12(prompt)};
      async function runDeepResearch(prompt,query){query=String(query||"").trim();if(!query){localReply(prompt,"Use **/deepresearch [question]** to run multi-query cited research.",false);return true}state.sending=true;render();try{var data=await postJson("/api/deep-research",{query:query,provider:state.settings.provider,reasoningPower:state.settings.reasoningPower},300000);state.sending=false;localReply(prompt,"**DEEP RESEARCH — MULTI-QUERY SYNTHESIS**\n\n"+data.response+"\n\nResearch passes: "+(data.queries||[]).length+" · unique sources: "+(data.sources||[]).length+" · model route: "+(data.modelKey||"max"),"Deep research complete, "+(state.settings.title||"sir")+".")}catch(error){state.sending=false;localReply(prompt,"Deep Research could not complete. "+error.message,false)}return true}
      async function runLocalFileSearch(prompt,query){var api=window.jarvisDesktop&&typeof window.jarvisDesktop.searchLocalFiles==="function"?window.jarvisDesktop:null;if(!api){localReply(prompt,"Local file search requires the genuine JARVIS EXE/MSI. The browser cannot inspect your computer files.",false);return true}state.sending=true;render();try{var result=await api.searchLocalFiles(query);state.sending=false;if(result.status==="cancelled"){localReply(prompt,"Local file search was cancelled. No folder was inspected.",false);return true}if(result.status!=="completed"){localReply(prompt,result.message||"Enter a filename or text to search for.",false);return true}var lines=result.results.map(function(item,index){return (index+1)+". **"+item.name+"**\n   "+item.path+(item.snippet?"\n   “"+item.snippet+"”":"")});localReply(prompt,"**LOCAL FILE SEARCH — USER-SELECTED FOLDER**\n\n"+(lines.length?lines.join("\n\n"):"No matching files were found.")+"\n\nInspected "+result.inspected+" files."+(result.truncated?" Search stopped at the safety limit.":""),false)}catch(error){state.sending=false;localReply(prompt,"Local file search was unavailable. "+error.message,false)}return true}
      var runSmartActionV13=runSmartAction;
      runSmartAction=async function(prompt){var match=String(prompt||"").match(/^\/(?:deepresearch|research-deep)\s*(.*)$/i);if(match)return runDeepResearch(prompt,match[1]);match=String(prompt||"").match(/^\/(?:filesearch|findfile|localfiles)\s+(.+)$/i)||String(prompt||"").match(/^find\s+(?:a\s+)?local\s+file\s+(?:named|containing)\s+(.+)$/i);if(match)return runLocalFileSearch(prompt,match[1]);return runSmartActionV13(prompt)};
      async function analyzePsdAttachment(file){var api=window.jarvisDesktop&&typeof window.jarvisDesktop.analyzePsd==="function"?window.jarvisDesktop:null;if(!api)throw new Error("Full PSD layer analysis requires the updated JARVIS EXE/MSI. The website alone cannot safely parse Photoshop layer structures.");var result=await api.analyzePsd({name:file.name,file:file.dataUrl});if(!result||!result.markdown)throw new Error("The desktop PSD parser returned no layer structure.");return result.markdown}
      async function analyzeOfficeAttachment(file){var api=window.jarvisDesktop&&typeof window.jarvisDesktop.analyzeOffice==="function"?window.jarvisDesktop:null;if(!api){var fallback=await postJson("/api/document",{name:file.name,file:file.dataUrl});if(!fallback||!fallback.markdown)throw new Error("Office conversion returned no content.");return fallback.markdown}var result=await api.analyzeOffice({name:file.name,file:file.dataUrl});if(!result||!result.markdown)throw new Error("The desktop Office parser returned no document structure.");return result.markdown}
      function learningChunks(text){var clean=String(text||"").replace(/\u0000/g,"").trim(),chunks=[];while(clean&&chunks.length<40){var end=Math.min(6000,clean.length);if(end<clean.length){var breakAt=Math.max(clean.lastIndexOf("\n\n",end),clean.lastIndexOf("\n",end));if(breakAt>3000)end=breakAt}chunks.push(clean.slice(0,end));clean=clean.slice(end).trim()}return chunks}
      function blockedLearningFile(name,text){return /(?:^|[\\/])(?:\.env(?:\.|$)|id_rsa|id_ed25519|credentials?\.(?:json|ini|txt)|secrets?\.(?:json|ya?ml|txt))$/i.test(String(name||""))||/-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----/.test(String(text||""))}
      async function learnAttachedFiles(prompt){var attached=state.attachments.slice();if(!attached.length){localReply(prompt,"Attach one or more files first, then type **/learnfiles**. JARVIS can learn from source code, SQL dumps, structured data, documents, and transcripts through approved Memory Vault entries.",false);return true}state.sending=true;render();var entries=[],skipped=[];try{for(var i=0;i<attached.length;i++){var file=attached[i],content="";if(file.kind==="image"){skipped.push(file.name+" (images are analyzed in chat, not stored as raw memory)");continue}if(file.kind==="audio"){try{content=(await postJson("/api/transcribe",{name:file.name,file:file.dataUrl})).transcript||""}catch(e){skipped.push(file.name+" (transcription failed)");continue}}else if(file.kind==="psd"){try{content=await analyzePsdAttachment(file)}catch(e){skipped.push(file.name+" (PSD analysis unavailable: "+e.message+")");continue}}else if(file.kind==="office"){try{content=await analyzeOfficeAttachment(file)}catch(e){skipped.push(file.name+" (Office analysis unavailable: "+e.message+")");continue}}else if(file.kind==="document"){try{content=(await postJson("/api/document",{name:file.name,file:file.dataUrl})).markdown||""}catch(e){skipped.push(file.name+" (safe conversion unavailable: "+e.message+")");continue}}else content=file.content||"";if(blockedLearningFile(file.name,content)){skipped.push(file.name+" (possible credential/private-key file)");continue}var chunks=learningChunks(content);for(var j=0;j<chunks.length&&entries.length<40;j++)entries.push({id:id("memory"),text:"Untrusted uploaded reference file: "+file.name+"\nChunk "+(j+1)+" of "+chunks.length+"\n\n"+chunks[j],source:"file",role:"reference",title:"File: "+file.name+" · "+(j+1)+"/"+chunks.length,createdAt:Date.now()})}state.sending=false;if(!entries.length){localReply(prompt,"No reusable text could be safely extracted. "+(skipped.length?"Skipped: "+skipped.join("; "):""),false);return true}if(!confirm("Save "+entries.length+" extracted reference chunks to JARVIS Memory Vault?\n\nUploaded content is untrusted reference data, never executable instructions. If sync is enabled, these memories may synchronize across your signed-in devices.")){localReply(prompt,"File learning cancelled. Nothing was saved.",false);return true}await memoryPutMany(entries);state.attachments=[];renderAttachments();await updateMemoryCount();localReply(prompt,"**FILE LEARNING COMPLETE**\nSaved "+entries.length+" approved chunks from "+attached.length+" attached file(s) to Memory Vault for RAG retrieval. JARVIS did not execute the files or change model weights."+(skipped.length?"\n\nSkipped: "+skipped.join("; "):""),false)}catch(error){state.sending=false;localReply(prompt,"File learning could not complete. "+error.message,false)}return true}
      var runSmartActionVFiles=runSmartAction;
      runSmartAction=async function(prompt){if(/^\/(?:learnfiles?|ingestfiles?)\s*$/i.test(String(prompt||""))||/^(?:learn|remember|study)\s+(?:from\s+)?(?:these|the|my)?\s*(?:attached|uploaded)\s+files?$/i.test(String(prompt||"").trim()))return learnAttachedFiles(prompt);return runSmartActionVFiles(prompt)};
      ...
      async function sendV2(){
        naturalOriginalPrompt="";var c=current(),conversationId=c.id,originalPrompt=q("#input").value.trim(),prompt=naturalIntentCommand(originalPrompt),directedMode=c.mode==="chat"?autoDirectedMode(originalPrompt):c.mode,stopAiProgress=function(){};if(prompt!==originalPrompt)naturalOriginalPrompt=originalPrompt;
        ...
        if(prompt.toLowerCase().indexOf("/remember ")===0){
          ...
          try{
            var now=Date.now();
            c.messages.push({id:id("msg"),role:"user",content:naturalOriginalPrompt||prompt,createdAt:now});
        ...
        var attachments=state.attachments.slice(),imageAttachment=attachments.find(function(a){return a.kind==="image"}),audioAttachments=attachments.filter(function(a){return a.kind==="audio"}),psdAttachments=attachments.filter(function(a){return a.kind==="psd"}),officeAttachments=attachments.filter(function(a){return a.kind==="office"}),documentAttachments=attachments.filter(function(a){return a.kind==="document"}),textAttachments=attachments.filter(function(a){return a.kind==="text"});
        if(audioAttachments.length){for(var ai=0;ai<audioAttachments.length;ai++){try{var transcriptData=await postJson("/api/transcribe",{audio:audioAttachments[ai].dataUrl});textAttachments.push({name:audioAttachments[ai].name+" — transcript",content:transcriptData.transcript})}catch(audioError){toast("Could not transcribe "+audioAttachments[ai].name+": "+audioError.message)}}}
        if(psdAttachments.length){for(var pi=0;pi<psdAttachments.length;pi++){try{var psdMarkdown=await analyzePsdAttachment(psdAttachments[pi]);textAttachments.push({name:psdAttachments[pi].name+" — PSD structure",content:psdMarkdown})}catch(psdError){toast("Could not analyze "+psdAttachments[pi].name+": "+psdError.message)}}}
        if(officeAttachments.length){for(var oi=0;oi<officeAttachments.length;oi++){try{var officeMarkdown=await analyzeOfficeAttachment(officeAttachments[oi]);textAttachments.push({name:officeAttachments[oi].name+" — Office structure",content:officeMarkdown})}catch(officeError){toast("Could not analyze "+officeAttachments[oi].name+": "+officeError.message)}}}
        if(documentAttachments.length){for(var di=0;di<documentAttachments.length;di++){try{var documentData=await postJson("/api/document",{name:documentAttachments[di].name,file:documentAttachments[di].dataUrl});textAttachments.push({name:documentAttachments[di].name+" — converted",content:documentData.markdown})}catch(documentError){toast("Could not convert "+documentAttachments[di].name+": "+documentError.message)}}}
        var context=textAttachments.length?"\n\nAttached context:\n"+textAttachments.map(function(a){return"--- "+a.name+" ---\n"+a.content}).join("\n\n"):"";
        var memory=await memorySearch(prompt);c=state.conversations.find(function(item){return item.id===conversationId})||current();conversationId=c.id;
        var msg={id:id("msg"),role:"user",content:prompt,createdAt:Date.now(),requestPending:true};
        ...
        try{
          ...
          if(directedMode==="image"){
            ...
            data=await postJson("/api/image",{prompt:prompt,quality:state.settings.reasoningPower})
            ...
            var research="",localWarnings=[];
            if(state.settings.webSearch){try{var researchData=await postJson("/api/research",{query:prompt});research=researchData.context||""}catch(localResearchError){localWarnings.push("Web research was unavailable, so local Ollama answered without live SearXNG evidence.")}}
            data={response:await askOllama({messages:c.messages,mode:directedMode},context,memory,research),warnings:localWarnings,toolsUsed:localWarnings.length?["web_search_unavailable"]:[]}
          }else{
            try{
              ...
              },state.settings.reasoningPower==="max"?300000:180000)
            }catch(cloudError){
          ...
          var warningText=Array.isArray(data.warnings)&&data.warnings.length?"\n\n**JARVIS NOTICE**\n"+data.warnings.map(function(item){return"• "+item}).join("\n"):"",reply={id:id("msg"),role:"assistant",content:(directedMode==="image"?(data.demo?"Vision module ready. Deploy with the Cloudflare Workers AI binding to generate this image.":"Image synthesis complete. You may download the result below."):(data.response||"No response was returned."))+warningText,image:data.image,reviewNeeded:!!(data.reflection&&data.reflection.uncertain),reviewed:false,toolsUsed:Array.isArray(data.toolsUsed)?data.toolsUsed:[],createdAt:Date.now()};
          var targetConversation=state.conversations.find(function(item){return item.id===conversationId})||current(),pendingMessage=targetConversation.messages.find(function(item){return item.id===msg.id});if(pendingMessage)pendingMessage.requestPending=false;targetConversation.messages.push(reply);targetConversation.updatedAt=Date.now();if(state.settings.autoSpeak)speak(reply.content)
        }catch(e){var failedReply={id:id("msg"),role:"assistant",content:"I am sorry, "+(state.settings.title||"sir")+". "+e.message,createdAt:Date.now()},failedConversation=state.conversations.find(function(item){return item.id===conversationId})||current(),failedPending=failedConversation.messages.find(function(item){return item.id===msg.id});if(failedPending)failedPending.requestPending=false;failedConversation.messages.push(failedReply);failedConversation.updatedAt=Date.now();if(state.settings.autoSpeak)speak(failedReply.content)}
      ...
      q("#logout").onclick=async function(){try{await stopWakeWordListener(true);if(state.settings.cloudSync)await syncNow(false,false);await fetch("/api/logout",{method:"POST"})}finally{location.replace("/")}};
      ...
      q("#maxBrain").onclick=function(){q("#provider").value="auto";q("#cloudModel").value="auto";q("#reasoningPower").value="max";q("#contextWindow").value="120000";q("#reflectionMode").checked=true;q("#webSearch").checked=true;q("#memoryEnabled").checked=true;q("#cloudSync").checked=true;q("#missionControlEnabled").checked=true;q("#screenVisionEnabled").checked=true;q("#itCopilotEnabled").checked=true;q("#proactiveBriefingEnabled").checked=true;q("#knowledgeAgentEnabled").checked=true;q("#autoSpeak").checked=true;q("#neuralVoice").checked=true;q("#concise").checked=false;q("#saveSettings").click();toast("Free-Max Unified JARVIS Brain activated. Hey JARVIS remains a separate privacy-controlled switch.")};
      q("#brainCheck").onclick=function(){closeSettings();runBrainSelfCheck("/aicheck")};
      ...
      q("#saveSettings").onclick=function(){state.settings={provider:q("#provider").value,cloudModel:q("#cloudModel").value,reasoningPower:q("#reasoningPower").value,contextWindow:Number(q("#contextWindow").value),ollamaUrl:q("#ollamaUrl").value.trim()||"http://localhost:11434/v1",ollamaModel:q("#ollamaModel").value.trim()||"qwen3:4b",webSearch:q("#webSearch").checked,memoryEnabled:q("#memoryEnabled").checked,cloudSync:q("#cloudSync").checked,reflectionMode:q("#reflectionMode").checked,weatherLocation:q("#weatherLocation").value.trim().slice(0,120)||"Iloilo City, Philippines",autoSpeak:q("#autoSpeak").checked,neuralVoice:q("#neuralVoice").checked,wakeWordEnabled:q("#wakeWordEnabled").checked,concise:q("#concise").checked,creativity:Number(q("#creativity").value),title:q("#title").value};state.syncMeta.settingsUpdatedAt=Date.now();if(!state.settings.cloudSync)clearTimeout(state.syncTimer);applySettings();save();render();closeSettings();toast("Configuration saved.");if(state.settings.cloudSync)syncNow(false,false)};
      ...
      document.addEventListener("keydown",function(e){if(e.key!=="Escape")return;if(q("#knowledgeModal").classList.contains("open")){e.preventDefault();closeKnowledgeAgent()}else if(q("#missionModal").classList.contains("open")){e.preventDefault();closeMissionControl()}else if(q("#modal").classList.contains("open")){e.preventDefault();closeSettings()}});
      q("#closeKnowledge").onclick=closeKnowledgeAgent;
      q("#knowledgeModal").onclick=function(event){if(event.target===q("#knowledgeModal"))closeKnowledgeAgent()};
      q("#researchKnowledge").onclick=function(){createKnowledgeUpdate(q("#knowledgeTopic").value)};
      q("#knowledgeTopic").onkeydown=function(event){if(event.key==="Enter"){event.preventDefault();createKnowledgeUpdate(this.value)}};
      q("#selectKnowledge").onclick=function(){var choices=Array.from(qa(".knowledge-choice")),select=choices.some(function(choice){return!choice.checked});choices.forEach(function(choice){choice.checked=select});this.textContent=select?"CLEAR SELECTION":"SELECT ALL"};
      q("#approveKnowledge").onclick=approveKnowledgeDraft;
      q("#rejectKnowledge").onclick=rejectKnowledgeDraft;
      ...
      var nativeWakeApi=desktopWakeApi();
      if(nativeWakeApi&&typeof nativeWakeApi.onWakeWord==="function")nativeWakeApi.onWakeWord(handleWakeWord);
      if(nativeWakeApi&&typeof nativeWakeApi.onWakeWordStatus==="function")nativeWakeApi.onWakeWordStatus(handleNativeWakeStatus);
      if("serviceWorker" in navigator)navigator.serviceWorker.register("/sw.js",{scope:"/"}).catch(function(){});
      document.addEventListener("visibilitychange",function(){if(document.visibilityState==="visible"){scheduleSync(300);scheduleWakeWord(300)}else if(state.settings.wakeWordEnabled){if(desktopWakeApi())updateWakeStatus("armed","JARVIS remains available through the Windows system-tray listener.");else{stopWakeWordListener(true);updateWakeStatus("paused","Browser wake-word detection pauses while the JARVIS page is hidden.")}}});
      window.addEventListener("beforeunload",function(){stopWakeWordListener(true)});
...
<body>
  ...
  <main class="login">
    ...
    <div class="secure"><b>●</b> SINGLE-USER SECURE SESSION · BUILD 1.15.1</div>
  </main>
  <script>
    (function(){
...
function textFromResult(result) {
  ...
  const choiceContent = Array.isArray(result.choices) && result.choices[0]
    ? result.choices[0].message && typeof result.choices[0].message.content === "string"
      ? result.choices[0].message.content
      : typeof result.choices[0].text === "string" ? result.choices[0].text : ""
    : "";
...
function aiErrorDetails(error) {
  ...
  const raw = String(source.message || (source.cause && source.cause.message) || error || "Cloudflare Workers AI request failed.");
  const normalized = raw.toLowerCase();
  const explicitCode = source.code || (source.cause && source.cause.code);
  const matchedCode = raw.match(/(?:error|code|status)\s*[:#-]?\s*(\d{3,6})/i);
  const code = String(explicitCode || (matchedCode && matchedCode[1]) || "").slice(0, 16);
  let classification = "unknown";
  if (/quota|allocation|3036|daily limit|neuron/i.test(normalized)) classification = "allocation";
  else if (/rate limit|too many requests|429/i.test(normalized)) classification = "rate_limit";
  else if (/billing|payment|required plan|paid plan|credits/i.test(normalized)) classification = "billing";
  else if (/unauthori[sz]ed|forbidden|permission|access denied|not enabled|authentication|401|403/i.test(normalized)) classification = "access";
  else if (/timeout|timed out/i.test(normalized)) classification = "timeout";
  else if (/model|capacity|unavailable|temporar|overloaded|not found|unsupported|internal server|bad gateway|service unavailable/i.test(normalized)) classification = "model";
  const message = raw
    .replace(/Bearer\s+[A-Za-z0-9._~+\/-]+/gi, "Bearer [REDACTED]")
    .replace(/([?&](?:key|token|secret|signature)=)[^&\s]+/gi, "$1[REDACTED]")
    .replace(/\b(?:sk|ghp|AIza)[A-Za-z0-9_-]{16,}\b/g, "[REDACTED]")
    .replace(/\b[a-f0-9]{24,}\b/gi, "[REDACTED_ID]")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 320);
...
function isQuotaOrAccessError(error) {
  return ["allocation", "rate_limit", "billing", "access"].includes(aiErrorDetails(error).classification);
}

function isAccountWideAiError(error) {
  return ["allocation", "rate_limit", "billing"].includes(aiErrorDetails(error).classification);
}

function aiErrorStatus(details) {
  if (details.classification === "allocation" || details.classification === "rate_limit") return 429;
  if (details.classification === "billing" || details.classification === "access") return 403;
  return 502;
}

function aiErrorUserMessage(details) {
  const suffix = details.code ? ` [Cloudflare ${details.code}]` : "";
  if (details.classification === "allocation") return `Cloudflare Workers AI reports that the daily free allocation is unavailable${suffix}. It resets at 00:00 UTC.`;
  if (details.classification === "rate_limit") return `Cloudflare Workers AI is temporarily rate-limiting requests${suffix}. Please retry shortly.`;
  if (details.classification === "billing") return `Cloudflare Workers AI rejected this route because of its billing or plan policy${suffix}.`;
  if (details.classification === "access") return `Cloudflare Workers AI rejected the account-bound inference request${suffix}. Run /aicheck for the exact sanitized diagnostic.`;
  if (details.classification === "timeout") return `Cloudflare Workers AI did not respond before the request timed out${suffix}.`;
  if (details.classification === "model") return `The selected Cloudflare model is temporarily unavailable${suffix}.`;
  return `Cloudflare Workers AI could not complete the request${suffix}.`;
}

function isRetryableModelError(error) {
  return /model|capacity|unavailable|temporar|timeout|timed out|overloaded|not found|unsupported|internal server|bad gateway|service unavailable/i
    .test(String(error && error.message || error || ""));
}

function modelCandidates(preferredKey) {
  const keys = FREE_TEXT_FALLBACKS[preferredKey] || FREE_TEXT_FALLBACKS.balanced;
  const seen = new Set();
...
async function runTextWithFallback(env, preferredKey, input) {
  const attempted = [];
  let lastError;
  for (const key of modelCandidates(preferredKey)) {
    const model = TEXT_MODELS[key];
    attempted.push(key);
    try {
      return { result: await env.AI.run(model, input), model, modelKey: key, attempted };
    } catch (error) {
      lastError = error;
      const classification = aiErrorDetails(error).classification;
      if (isAccountWideAiError(error) || (classification !== "access" && !isRetryableModelError(error))) throw error;
    }
  }
  throw lastError || new Error("No free Cloudflare text route was available.");
}

export function fitConversationMessages(rawMessages, maxMessages, characterBudget) {
  const budget = Math.max(1_000, Number(characterBudget) || 1_000);
  const valid = Array.isArray(rawMessages)
    ? rawMessages.slice(-Math.max(1, Number(maxMessages) || 1)).filter((message) =>
        message && typeof message === "object" && ["user", "assistant"].includes(message.role) && typeof message.content === "string")
    : [];
  const fitted = [];
  let remaining = budget;
  for (let index = valid.length - 1; index >= 0 && remaining > 0; index -= 1) {
    const message = valid[index];
    const maximum = Math.min(24_000, remaining);
    const content = message.content.slice(0, maximum);
    if (!content) continue;
    fitted.push({ role: message.role, content });
...
function boundedText(value, budget) {
  return typeof value === "string" && budget > 0 ? value.slice(0, Math.floor(budget)) : "";
}

function shouldReflect(prompt, mode, reasoningPower, enabled) {
  if (!enabled) return false;
  const text = String(prompt || "");
  if (mode === "code") return true;
  if (text.length >= 240) return true;
  if (/\b(analy[sz]e|reason|compare|investigate|root cause|architecture|strategy|prove|audit|security|medical|legal|financial|diagnos|research|plan|multi[- ]step|calculate|complex|verify|fact[- ]check)\b/i.test(text)) return true;
  return reasoningPower !== "standard" && /\b(explain|recommend|evaluate|design|troubleshoot|decide)\b/i.test(text);
}
...
async function readBody(request, maxBytes = 1_000_000) {
  ...
  if (new TextEncoder().encode(text).byteLength > maxBytes) throw new Error("Request context is too large.");
  try {
...
async function searchWebResults(query, env) {
  const configured = typeof env.SEARXNG_URL === "string" ? env.SEARXNG_URL.trim() : "";
  if (!configured) {
    throw new Error("Web research is enabled, but SEARXNG_URL is not configured as a Cloudflare Worker secret. Run CONFIGURE_KNOWLEDGE_AGENT.bat.");
  }
  let url;
  try {
    ...
    ? data.results
        .filter((item) => {
          ...
          try {
            ...
            return ["https:", "http:"].includes(resultUrl.protocol) && !resultUrl.username && !resultUrl.password;
          } catch { return false; }
        })
        .slice(0, 6)
        .map((item, index) => ({
          ...
          url: new URL(item.url).href.slice(0, 2_000),
...
async function runKnowledgeStructuredStep(env, { label, messages, schema, maxTokens, temperature, topP }) {
  ...
  let firstError;
  try {
    result = await runStep(messages, temperature);
  ...
  try {
    const retried = await runStep(retryMessages, 0);
...
function normalizeKnowledgeProposal(value, sources) {
  const item = value && typeof value === "object" ? value : {};
  const fact = String(item.fact || "").replace(/\s+/g, " ").trim().slice(0, 1_200);
  const reason = String(item.reason || "").replace(/\s+/g, " ").trim().slice(0, 600);
  const confidence = item.confidence === "high" ? "high" : "medium";
  const indexes = Array.from(new Set((Array.isArray(item.sourceIndexes) ? item.sourceIndexes : [])
    .map((index) => Number(index)).filter((index) => Number.isInteger(index) && index >= 1 && index <= sources.length)));
  const selectedSources = indexes.map((index) => sources[index - 1]).filter(Boolean);
  const domains = new Set(selectedSources.map((source) => {
    try { return new URL(source.url).hostname.toLowerCase().replace(/^www\./, ""); } catch { return ""; }
  }).filter(Boolean));
  if (fact.length < 20 || selectedSources.length < 2 || domains.size < 2) return null;
  return { fact, reason, confidence, sources: selectedSources };
}
...
async function knowledgeUpdate(request, env) {
  try {
    ...
    const searches = await Promise.all([
      ...
      searchWebResults(`${topic} official documentation latest current`, env),
    ]);
    const unique = [];
    const seen = new Set();
    searches.flat().forEach((source) => {
      const key = String(source.url || "").replace(/#.*$/, "");
      if (!key || seen.has(key) || unique.length >= 10) return;
      seen.add(key);
      unique.push({ ...source, index: unique.length + 1 });
    });
    const sourceDomains = new Set(unique.map((source) => {
      try { return new URL(source.url).hostname.toLowerCase().replace(/^www\./, ""); } catch { return ""; }
    }).filter(Boolean));
    if (unique.length < 2 || sourceDomains.size < 2) {
      return json({ error: "The Knowledge Update Agent could not find two independent source domains. Nothing was proposed or saved." }, 422);
    }
    ...
    if (!proposals.length) {
      return json({ topic, summary: "No claims met the two-source verification rule. Nothing was proposed or saved.", generatedAt: Date.now(), proposals: [], sources: unique });
    }

    const criticEvidence = proposals.map((proposal, index) => ({
      index,
      fact: proposal.fact,
    ...
    return json({
      topic,
      summary: String(criticData.summary || generatedData.summary || "Review the verified proposals before saving.").slice(0, 1_000),
      generatedAt: Date.now(),
      proposals: approved,
      sources: unique,
      policy: "No proposal is saved until the signed-in user explicitly approves it.",
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Knowledge update failed.";
    return json({ error: "Knowledge update error: " + message }, 502);
  }
}

function autoModelForTask(mode, prompt, reasoningPower) {
  const text = String(prompt || "").toLowerCase();
  if (mode === "code" || /\b(code|debug|script|program|powershell|javascript|typescript|python|sql|regex|api)\b/.test(text)) return "code";
  if (reasoningPower === "max" || /\b(analy[sz]e|reason|compare|investigate|root cause|architecture|strategy|prove|calculate|complex|step[- ]by[- ]step)\b/.test(text)) return "reasoning";
  if (reasoningPower === "deep") return "max";
  return "balanced";
}
...
async function chat(request, env) {
  try {
    ...
    const requestedContext = CONTEXT_CHARACTER_BUDGETS.includes(Number(body.contextWindow)) ? Number(body.contextWindow) : 32_000;
    const messageCount = reasoningPower === "max" ? 30 : reasoningPower === "deep" ? 24 : 18;
    const rawMessages = Array.isArray(body.messages)
      ? body.messages.slice(-messageCount).filter((message) =>
          message && typeof message === "object" && ["user", "assistant"].includes(message.role) && typeof message.content === "string")
      : [];
    if (!rawMessages.length) return json({ error: "A message is required." }, 400);
    ...
    const lastPrompt = rawMessages[rawMessages.length - 1].content.slice(0, 24_000);
    const requestedModel = Object.prototype.hasOwnProperty.call(TEXT_MODELS, body.modelKey) ? body.modelKey : "auto";
    const modelKey = requestedModel === "auto" ? autoModelForTask(mode, lastPrompt, reasoningPower) : requestedModel;
    const effectiveContext = mode === "code" && modelKey === "code" ? Math.min(requestedContext, 100_000) : requestedContext;
    const context = boundedText(body.context, effectiveContext * 0.30);
    const memory = boundedText(body.memory, effectiveContext * 0.12);
    const warnings = [];
    let webContextRaw = "";
    if (body.webSearch) {
      try { webContextRaw = await searchWeb(lastPrompt, env); }
      catch { warnings.push("Web research was unavailable, so this answer was generated without live SearXNG evidence."); }
    }
    const webContext = boundedText(webContextRaw, effectiveContext * 0.16);
    let semanticContextRaw = "";
    if (body.semanticMemory !== false) {
      try { semanticContextRaw = await semanticMemorySearch(lastPrompt, env); } catch {}
    }
    const semanticContext = boundedText(semanticContextRaw, effectiveContext * 0.12);
    const systemReserve = 6_000;
    const messageBudget = Math.max(4_000, effectiveContext - systemReserve - context.length - memory.length - webContext.length - semanticContext.length);
    const messages = fitConversationMessages(rawMessages, messageCount, messageBudget);
    if (!messages.length) return json({ error: "The conversation exceeded the selected context budget." }, 400);
    ...
    const generatorInput = {
      ...
      max_tokens: reasoningPower === "max" ? (mode === "code" ? 3_200 : 2_600) : reasoningPower === "deep" ? (mode === "code" ? 2_200 : 1_800) : (mode === "code" ? 1_400 : 1_100),
      temperature,
      top_p: 0.9,
    };
    if (imageData) generatorInput.image = imageData;
    const generatorRoute = imageData
      ? { result: await env.AI.run(selectedModel, generatorInput), model: selectedModel, modelKey: "vision", attempted: ["vision"] }
      : await runTextWithFallback(env, modelKey, generatorInput);
    let response = textFromResult(generatorRoute.result);
    ...
    if (shouldReflect(lastPrompt, mode, reasoningPower, body.reflectionMode === true)) {
      try {
        const criticRoute = await runTextWithFallback(env, reasoningPower === "max" ? "reasoning" : "balanced", {
          messages: [
        ...
        const review = textFromResult(criticRoute.result).trim();
        ...
        if (verdict !== "PASS") {
          ...
          const revised = imageData
            ? textFromResult(await env.AI.run(selectedModel, revisionInput))
            : textFromResult((await runTextWithFallback(env, generatorRoute.modelKey, revisionInput)).result);
          if (revised) {
            response = revised;
    ...
    if (warnings.length) toolsUsed.push("web_search_unavailable");
    if (semanticContext) toolsUsed.push("semantic_memory");
    if (reflection.used) toolsUsed.push("reflection");
    if (generatorRoute.attempted.length > 1) toolsUsed.push("model_fallback");
    const contextBudget = {
      requested: requestedContext,
      effective: effectiveContext,
      used: systemReserve + messages.reduce((total, message) => total + message.content.length, 0) + context.length + memory.length + webContext.length + semanticContext.length,
      messages: messages.reduce((total, message) => total + message.content.length, 0),
      attachments: context.length,
      memory: memory.length + semanticContext.length,
      research: webContext.length,
    };
    return json({ response, modelKey: generatorRoute.modelKey, modelUsed: generatorRoute.model, route: generatorRoute.attempted, contextBudget, reflection, warnings, toolsUsed });
  } catch (error) {
    const diagnostic = aiErrorDetails(error);
    return json({ error: aiErrorUserMessage(diagnostic), diagnostic: { classification: diagnostic.classification, code: diagnostic.code } }, aiErrorStatus(diagnostic));
  }
}
...
async function deepResearch(request, env) {
  try {
    ...
    if (batches.length < 2) throw new Error("Fewer than two independent research searches completed. Check SearXNG and retry.");
    const seen = new Set();
    const sources = [];
    for (const batch of batches) {
      for (const result of batch) {
        ...
        seen.add(result.url);
        sources.push({ ...result, index: sources.length + 1 });
        if (sources.length >= 18) break;
      }
      if (sources.length >= 18) break;
    }
    const evidence = formatWebResults(sources);
    ...
    const route = await runTextWithFallback(env, modelKey, {
      messages: [{ role: "system", content: system }, ...messages],
      max_tokens: body.reasoningPower === "max" ? 3_600 : 2_600,
      temperature: 0.15,
      top_p: 0.85,
    });
    const response = textFromResult(route.result);
    if (!response) throw new Error("The research model returned an empty response.");
    const toolsUsed = ["deep_research", "multi_query_search"];
    if (route.attempted.length > 1) toolsUsed.push("model_fallback");
    return json({ response, sources, queries, completedQueries: batches.length, modelKey: route.modelKey, modelUsed: route.model, route: route.attempted, toolsUsed });
  } catch (error) {
...
async function image(request, env) {
  try {
    ...
    if (body.quality === "deep" || body.quality === "max") {
      try {
        const enhanced = textFromResult((await runTextWithFallback(env, body.quality === "max" ? "max" : "fast", {
          messages: [
            { role: "system", content: "Rewrite the user's image request as one production-ready visual prompt. Preserve all requested subjects and constraints. Add useful composition, lighting, material, camera, color, and quality detail, but do not add copyrighted characters, artist imitation, or unwanted text. Return only the enhanced prompt." },
            { role: "user", content: prompt },
          ],
          max_tokens: 320,
          temperature: 0.35,
        })).result);
        if (enhanced) generationPrompt = enhanced.slice(0, 2_048);
      } catch {}
    }
    ...
    return json({ image: "data:image/jpeg;base64," + result.image, enhancedPrompt: generationPrompt !== prompt ? generationPrompt : "" });
  } catch (error) {
...
async function transcribeAudio(request, env) {
  try {
    const body = await readBody(request, 18_000_000);
    const match = typeof body.audio === "string"
      ? body.audio.match(/^data:audio\/(?:mpeg|mp3|wav|wave|webm|mp4|m4a);base64,([A-Za-z0-9+/=]+)$/i)
      : null;
    if (!match || match[1].length > 16_000_000) return json({ error: "Attach an MP3, WAV, WebM, MP4, or M4A audio file no larger than 12 MB." }, 400);
    if (!env.AI) return json({ error: "Audio transcription requires the Cloudflare Workers AI binding." }, 503);
    const result = await env.AI.run(SPEECH_MODEL, {
      audio: Array.from(decodeBase64(match[1])),
      task: "transcribe",
      language: typeof body.language === "string" ? body.language.slice(0, 12) : undefined,
      vad_filter: true,
      beam_size: 5,
    });
    const transcript = String(result && (result.text || result.transcription_info && result.transcription_info.text) || "").trim();
    if (!transcript) throw new Error("The speech model returned no transcript.");
    return json({ transcript: transcript.slice(0, 120_000), model: SPEECH_MODEL });
  } catch (error) {
    return json({ error: "Audio transcription error: " + (error instanceof Error ? error.message : "Transcription failed.") }, 502);
  }
}

async function convertDocument(request, env) {
  try {
    const body = await readBody(request, 12_000_000);
    const name = typeof body.name === "string" ? body.name.replace(/[\\/]/g, "_").slice(0, 240) : "document";
    const match = typeof body.file === "string" ? body.file.match(/^data:([^;,]{1,120});base64,([A-Za-z0-9+/=]+)$/) : null;
    if (!match || match[2].length > 11_000_000) return json({ error: "The document is missing or exceeds the 8 MB conversion limit." }, 400);
    if (!env.AI || typeof env.AI.toMarkdown !== "function") return json({ error: "Safe binary-file conversion requires the current Cloudflare Workers AI binding. Export unsupported formats to TXT, SQL, CSV, JSON, PDF, DOCX, XLSX, or PPTX." }, 503);
    const result = await env.AI.toMarkdown({ name, blob: new Blob([decodeBase64(match[2])], { type: match[1] }) });
    const converted = Array.isArray(result) ? result[0] : result;
    if (!converted || converted.format === "error" || typeof converted.data !== "string") throw new Error(converted && converted.error || "Cloudflare returned no converted document text.");
    return json({ name, markdown: converted.data.slice(0, 120_000), tokens: Number(converted.tokens || 0), mimetype: String(converted.mimetype || match[1]) });
  } catch (error) {
    return json({ error: "Document conversion error: " + (error instanceof Error ? error.message : "Conversion failed.") }, 502);
  }
}
...
async function speech(request, env) {
  try {
    const body = await readBody(request);
    const text = typeof body.text === "string" ? body.text.trim().slice(0, 1_800) : "";
    if (!text) return json({ error: "Text is required for neural speech." }, 400);
    if (!env.AI) return json({ error: "Neural speech requires the Cloudflare Workers AI binding." }, 503);
    const result = await env.AI.run(TTS_MODEL, { prompt: text, lang: "en" });
    if (result instanceof ReadableStream || result instanceof ArrayBuffer || ArrayBuffer.isView(result)) {
      return new Response(result, { headers: { "content-type": "audio/mpeg", "cache-control": "no-store", ...SECURITY_HEADERS } });
    }
    if (result && typeof result.audio === "string") {
      return new Response(decodeBase64(result.audio), { headers: { "content-type": "audio/mpeg", "cache-control": "no-store", ...SECURITY_HEADERS } });
    }
    throw new Error("The neural voice model returned no audio.");
  } catch (error) {
    return json({ error: "Neural voice error: " + (error instanceof Error ? error.message : "Speech generation failed.") }, 502);
  }
}

async function probeBrainRoute(env, key, model) {
  const startedAt = Date.now();
  try {
    const result = await env.AI.run(model, {
      messages: [
        { role: "system", content: "This is a JARVIS health probe. Reply with exactly READY." },
        { role: "user", content: "Health probe" },
      ],
    });
    const output = textFromResult(result).trim();
    if (!output) throw new Error("The model returned an empty health-probe response.");
    return { key, model, ok: true, latencyMs: Date.now() - startedAt, output: output.slice(0, 40), classification: "ready", code: "", message: "Live inference succeeded." };
  } catch (error) {
    const diagnostic = aiErrorDetails(error);
    return {
      key,
      ...
      message: diagnostic.message || aiErrorUserMessage(diagnostic),
    };
  }
}

async function brainCheck(request, env) {
  const startedAt = Date.now();
  const bindings = {
    cloudflareAI: Boolean(env.AI && typeof env.AI.run === "function"),
    documentConversion: Boolean(env.AI && typeof env.AI.toMarkdown === "function"),
    encryptedSync: syncConfigured(env),
    semanticMemory: Boolean(env.JARVIS_VECTORIZE && typeof env.JARVIS_VECTORIZE.query === "function"),
    webResearch: Boolean(typeof env.SEARXNG_URL === "string" && /^https:\/\//i.test(env.SEARXNG_URL.trim())),
  };
  let probes = [];
  let live = { ok: false, latencyMs: 0, modelKey: "", model: "", route: [], classification: "configuration", code: "", message: "Cloudflare Workers AI is not bound." };
  if (bindings.cloudflareAI) {
    const routes = [
      ["general", TEXT_MODELS.balanced],
      ["code", TEXT_MODELS.code],
      ["reasoning", TEXT_MODELS.reasoning],
      ["vision-text", VISION_MODEL],
    ];
    for (const [key, model] of routes) probes.push(await probeBrainRoute(env, key, model));
    const general = probes[0];
    const firstReady = probes.find((probe) => probe.ok);
    const representative = general && general.ok ? general : firstReady || general;
    if (representative) {
      live = {
        ok: Boolean(representative.ok),
        latencyMs: representative.latencyMs,
        modelKey: representative.key,
        model: representative.model,
        route: probes.filter((probe) => probe.ok).map((probe) => probe.key),
        classification: representative.classification,
        code: representative.code,
        message: representative.ok ? "An account-bound Cloudflare model response was received." : representative.message,
      };
    }
  }
  const readyCount = probes.filter((probe) => probe.ok).length;
  const status = !bindings.cloudflareAI ? "configuration_required" : readyCount === probes.length && readyCount > 0 ? "ready" : "degraded";
  return json({
    service: "JARVIS Unified Brain",
    build: BUILD_VERSION,
    status,
    checkedAt: new Date().toISOString(),
    live,
    probes,
    summary: { testedRoutes: probes.length, readyRoutes: readyCount, failedRoutes: probes.length - readyCount, accountInferenceReached: readyCount > 0, totalLatencyMs: Date.now() - startedAt },
    bindings,
    models: {
      general: TEXT_MODELS.balanced,
      reasoning: TEXT_MODELS.reasoning,
    ...
    context: { maximumManagedCharacters: 120_000, globallyBudgeted: true },
    freePolicy: { cloudflareNeuronsPerDay: 10_000, resetsAtUtc: "00:00", paidOnlyRoutesEnabled: false },
  });
}
...
export default {
  async fetch(request, env = {}, ctx = {}) {
    ...
    if (url.pathname === "/api/health") {
      return json({ service: "JARVIS", status: "online", build: BUILD_VERSION, ai: Boolean(env.AI) });
    }
    ...
    if (request.method === "POST" && url.pathname === "/api/brain-check") return brainCheck(request, env);
    ...
    if (request.method === "POST" && url.pathname === "/api/deep-research") return deepResearch(request, env);
    if (request.method === "POST" && url.pathname === "/api/knowledge-update") return knowledgeUpdate(request, env);
    ...
    if (request.method === "POST" && url.pathname === "/api/transcribe") return transcribeAudio(request, env);
    if (request.method === "POST" && url.pathname === "/api/document") return convertDocument(request, env);
    if (request.method === "POST" && url.pathname === "/api/speech") return speech(request, env);

...
<body>
  ...
  <div class="modal" id="modal">
    <section class="settings" role="dialog" aria-modal="true" aria-label="JARVIS settings">
      ...
      <label class="setting toggle"><span class="setting-copy"><strong>Always listen for “Hey JARVIS”</strong><small>The Windows EXE/MSI continues listening from its visible system-tray icon when minimized and comes forward after the wake phrase. Edge/Chrome listening pauses while the page is hidden and may use an online speech service.</small></span><input id="wakeWordEnabled" type="checkbox"><i></i></label>
  ...
  <script>
    (function(){
      ...
      function updateWakeStatus(status,message){var badge=q("#wakeStatus"),label=badge&&badge.querySelector("b");if(!badge||!label)return;badge.className="wake-status";badge.hidden=status==="off"&&!state.settings.wakeWordEnabled;if(status==="armed"){badge.classList.add("armed");label.textContent="HEY JARVIS ARMED"}else if(status==="awake"){badge.classList.add("awake");label.textContent="JARVIS AWAKE"}else if(status==="error"||status==="unavailable"){badge.classList.add("error");label.textContent=status==="unavailable"?"WAKE WORD UNAVAILABLE":"WAKE WORD ERROR"}else if(status==="paused"){label.textContent="WAKE WORD PAUSED"}else if(status==="starting"){label.textContent="STARTING WAKE WORD…"}else{label.textContent="HEY JARVIS OFF"}if(message)badge.title=message;else badge.removeAttribute("title");q("#voiceStatus").textContent=status==="armed"?"WAKE ARMED":state.settings.autoSpeak?"ACTIVE":"STANDBY";q("#voiceStatus").classList.toggle("on",status==="armed"||state.settings.autoSpeak)}
      async function stopWakeWordListener(preserveSetting){clearTimeout(wakeRestartTimer);wakeRestartTimer=null;wakeStarting=false;var browser=wakeRecognition;wakeRecognition=null;if(browser){browser.onend=null;browser.onerror=null;try{browser.abort()}catch(error){}}var api=desktopWakeApi();if(api){try{await api.setWakeWordEnabled(false)}catch(error){}}wakeUsingNative=false;if(!preserveSetting){state.settings.wakeWordEnabled=false;q("#wakeWordEnabled").checked=false;saveLocalOnly();updateWakeStatus("off")}}
      ...
      function startBrowserWakeWord(){var R=window.SpeechRecognition||window.webkitSpeechRecognition;if(!R){updateWakeStatus("unavailable","This browser does not provide continuous speech recognition. Use the JARVIS Windows EXE/MSI or current Edge/Chrome.");toast("Hey JARVIS is unavailable here. Use the Windows app or current Edge/Chrome.");return false}var instance=new R();wakeRecognition=instance;instance.continuous=true;instance.interimResults=false;instance.lang="en-US";instance.onstart=function(){if(wakeRecognition===instance)updateWakeStatus("armed","Listening only for the Hey JARVIS wake phrase while this app is open.")};instance.onresult=function(event){for(var i=event.resultIndex;i<event.results.length;i++){if(!event.results[i].isFinal)continue;var phrase=wakePhraseResult(event.results[i][0].transcript);if(phrase){handleWakeWord(phrase);break}}};instance.onerror=function(event){if(wakeRecognition!==instance)return;var reason=String(event.error||"");if(reason==="aborted"||reason==="no-speech")return;if(reason==="not-allowed"||reason==="service-not-allowed"){state.settings.wakeWordEnabled=false;q("#wakeWordEnabled").checked=false;saveLocalOnly();updateWakeStatus("error","Microphone access was denied.");toast("Hey JARVIS was turned off because microphone access was denied.");return}updateWakeStatus("error","Speech recognition stopped: "+reason)};instance.onend=function(){if(wakeRecognition===instance)wakeRecognition=null;if(state.settings.wakeWordEnabled&&!wakeHandling)scheduleWakeWord(900)};try{instance.start();return true}catch(error){wakeRecognition=null;updateWakeStatus("error",error.message);scheduleWakeWord(1800);return false}}
      async function startWakeWordListener(){wakeRestartTimer=null;if(!state.settings.wakeWordEnabled)return;if(state.speaking||state.listening||state.sending||wakeHandling){scheduleWakeWord(900);return}if(wakeRecognition||wakeUsingNative||wakeStarting)return;var api=desktopWakeApi();if(api){wakeStarting=true;updateWakeStatus("starting","Starting the local Windows wake-word listener.");try{var result=await api.setWakeWordEnabled(true);wakeStarting=false;wakeUsingNative=true;updateWakeStatus(result&&result.status==="listening"?"armed":"starting","Windows speech recognition is waiting for Hey JARVIS.");return}catch(error){wakeStarting=false;wakeUsingNative=false}}startBrowserWakeWord()}
      function syncWakeWordSetting(){if(state.settings.wakeWordEnabled)scheduleWakeWord(100);else stopWakeWordListener(true).then(function(){updateWakeStatus("off")})}
      function handleNativeWakeStatus(payload){if(!state.settings.wakeWordEnabled)return;var status=String(payload&&payload.status||"");if(status==="listening"){wakeStarting=false;wakeUsingNative=true;updateWakeStatus("armed","Windows speech recognition is listening locally for Hey JARVIS.");return}if(status==="error"){wakeStarting=false;wakeUsingNative=false;updateWakeStatus("error",payload&&payload.message||"Windows wake-word recognition stopped.");if(!startBrowserWakeWord())toast("Hey JARVIS needs an English Windows speech-recognition language.")}}
...
<body>
  ...
  <main class="login">
    ...
    <div class="secure"><b>●</b> SINGLE-USER SECURE SESSION · BUILD 1.12.1</div>
  </main>
  <script>
    (function(){
...
export default {
  async fetch(request, env = {}, ctx = {}) {
    ...
    if (url.pathname === "/api/health") {
      return json({ service: "JARVIS", status: "online", build: "1.12.1", ai: Boolean(env.AI) });
    }
